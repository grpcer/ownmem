...
全体の利便性です。マシン上のすべてのリポジトリに同じ 3 つの skill を
...
1 つのプラグイン、3 つの skill、1 組の名前:

| Skill | Claude Code | Codex CLI | 役割 |
| --- | --- | --- | --- |
| `recall` | `/ownmem:recall` | `ownmem:recall` | コードを変更する前にメモリを recall する |
...
ここでも skill はセッション開始時に読み込まれます。`$` の skill ピッカーで
