...
| 公開基準 | v0.1.2 鎖定合成基準：Recall@1 100%、P95 2.46 ms；這是回歸證據，不代表實際使用準確率 |
| 授權 | Apache-2.0 |
...
[CONTRIBUTING.md](./CONTRIBUTING.md)：讓預設召回保持確定性、在本機執行且不使用
模型；每項檢索變更都要加入回歸測試案例；送出審閱要求前，請執行 `npm test` 與
`npm run benchmark:release`。安全性問題請透過 [SECURITY.md](./SECURITY.md) 回報。
...
OwnMem 採用 Apache-2.0 授權。分享成品或發布版本前，請先閱讀 `PRIVACY.md`、
`SECURITY.md` 與 `RELEASE.md`。
