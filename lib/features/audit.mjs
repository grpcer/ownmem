...
function parseArgs(args) {
  const options = { root: process.cwd(), memoryDir: null, writeLock: false, json: false, observability: true };
  ...
  // Resolved after the loop so that --root is final, and through the one resolver so a module reached
  // directly can never gate a project against a directory the project does not use.
...
export function runCli(args = process.argv.slice(2)) {
  ...
  if (options.observability) {
    ...
    if (!recorded.written && recorded.error) {
      process.stderr.write(`ownmem audit: local observability skipped: ${recorded.error}\n`);
    }
  }
...
if (isMemoryCliEntry(import.meta.url)) {
  try {
    ...
    process.stderr.write(`ownmem audit: ${error.message}\n`);
    process.exitCode = 1;
  }
}
