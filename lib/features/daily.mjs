...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    else if (argument === '--no-fetch') options.fetch = false; // ignored: 0.6.0 flag, no longer used
    else if (argument === '--no-commit') options.commit = false; // ignored: 0.6.0 flag, no longer used
...
The unattended daily pass. Archives every finished day that is still on disk into the local
observability tree, drops expired raw events, and inspects those packages. It is wired to session
start and session stop, and runs at most once per UTC day; the later triggers cost nothing.
...
starts past the deadline is skipped rather than started -- nothing here may block a session.

Options:
  --host NAME             Which agent is running this (${MEMORY_DECLARABLE_AGENTS.join(', ')})
  --source NAME           What triggered it, recorded in the trace (session-start, stop)
