...
function parseArgs(args) {
  ...
  // Resolved after the loop so that --root is final; the .claude/memory default made every console
  // request throw on an installation that keeps its memory anywhere else.
...
export async function runCli(rawArgs = process.argv.slice(2), io = { stdout: process.stdout, stderr: process.stderr }) {
  ...
  // The embed endpoints compile through the same snapshot directory as the CLI and the runtime;
  // leaving this unset silently re-created the retired .local-test/memory-index copy.
...
if (isMemoryCliEntry(import.meta.url)) {
  runCli().then(code => {
    ...
    process.stderr.write(`ownmem dashboard: ${error.message}\n`);
    process.exitCode = 1;
  });
}
