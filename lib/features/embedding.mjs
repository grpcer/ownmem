...
Global options:
  ...
  --index-dir PATH   Local compiled memory index (derived from --memory-dir)
...
ab options:
  ...
  --cases-file PATH  Recall cases fixture (--cases is a compatibility alias); without one,
                     conventional locations are searched and the run reports a skip
...
function parseArgs(rawArgs) {
  ...
  for (let index = 1; index < rawArgs.length; index += 1) {
    ...
    if (argument === '--help' || argument === '-h') options.help = true;
      ...
      index += 1;
    } else if (argument === '--cases-file' || argument === '--cases') {
      if (command !== 'ab') throw new Error(`${argument} is only valid with the ab command`);
...
async function runAbCommand(options, io, dependencies, compile) {
  ...
  if (result.status === 'skipped') {
    // A skip is a reportable outcome, not a silent success: the guard did not run, and the line
    // has to say so in the same place a passing guard would have printed its numbers.
    if (!options.json) io.stdout.write(`memory embedding A/B: skipped (${result.reason})\n`);
    return;
  }
  if (!options.json) {
    ...
    io.stdout.write(
      ...
      // The verdict leads and `safe` follows it, because `safe=true` alone is what a tie on a
      // saturated corpus prints, and that sentence was read as a result for two weeks.
      + `verdict=${guard.verdict}; safe=${guard.recommended_weight_safe}; report ${result.report_file}\n`
      + (guard.verdict === 'uninformative'
        ? '  the baseline already scores at the ceiling on every metric, so this run could only have detected harm\n'
        : ''),
    );
  }
}
...
if (isMemoryCliEntry(import.meta.url)) {
  runCli().then((code) => {
    ...
    process.stderr.write(`memory-embed: ${error.message}\n`);
    process.exitCode = 1;
  });
}
