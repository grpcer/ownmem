...
import { formatSwapNomination, nominateSwapOut } from '../memory-swap-nomination.mjs';
...
export function collectMemoryHealth(options = {}) {
  ...
  // The Edit/Write hook reaches a memory through one route only: the compiler indexes `path` and
  // `basename` exact anchors out of code_evidence[].path and nowhere else. A topic without one is
  // unreachable from that surface no matter what it says, so this is the hook's actual coverage and
  // belongs in the report rather than in a one-off count. Split by type because the gap is not
  // uniform: a preference or a feedback rule is about how to work, has no code it is evidence
  // about, and is not missing anything by having none.
  const pathAnchorExempt = new Set(['preference', 'feedback']);
  let topicsWithPathAnchor = 0;
  let topicsWithoutPathAnchorAnchorable = 0;
  let topicsWithoutPathAnchorExempt = 0;
  for (const topic of validTopics) {
    ...
    if (metadata.code_evidence.some((evidence) => typeof evidence.path === 'string' && evidence.path.length > 0)) {
      topicsWithPathAnchor += 1;
    } else if (pathAnchorExempt.has(metadata.type)) {
      topicsWithoutPathAnchorExempt += 1;
    } else {
      topicsWithoutPathAnchorAnchorable += 1;
    }
  ...
  // And the same for a ruled-on coverage gap: health reporting it as open work after the ruling would
  // put this command and memory-feedback-review at different numbers for the same queue.
  const dismissalFile = options.dismissalFile
    || path.join(root, '.local-test/memory-miss-dismissal-receipts.jsonl');
  const dismissedLines = new Set(readMissDismissalReceipts(dismissalFile).entries.map((entry) => entry.feedback_line));
  ...
  return {
    ...
    evidence: {
      ...
      path_anchor_reach: {
        reachable: topicsWithPathAnchor,
        unreachable: topicsWithoutPathAnchorAnchorable,
        exempt: topicsWithoutPathAnchorExempt,
      },
    },
    // Who should leave when the count quota binds. Reported unconditionally rather than only on a
    // full corpus: the answer is what makes "archive one to add one" a decision instead of a wall,
    // and it is worth being able to look at it before hitting the wall.
    swap_nomination: nominateSwapOut({
      memoryDirectory: absoluteMemoryDir,
      observabilityDirectory: path.resolve(root, options.observabilityDirectory || '.local-test/memory-observability'),
      topics: validTopics,
    }),
    evaluation: {
    ...
    feedback: {
      ...
      ...summarizeFeedback(feedback, { resolvedLines, dismissedLines }),
    },
    usage: {
...
async function run() {
  ...
  process.stdout.write(
    `Path-anchor reach (Edit/Write hook): ${report.evidence.path_anchor_reach.reachable} reachable, `
    + `${report.evidence.path_anchor_reach.unreachable} unreachable, `
    + `${report.evidence.path_anchor_reach.exempt} exempt (preference/feedback)\n`,
  );
  process.stdout.write(formatSwapNomination(report.swap_nomination));
  process.stdout.write(
