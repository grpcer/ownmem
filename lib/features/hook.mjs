...
export function spawnUnattendedEvolution(root) {
  ...
  try {
    const child = spawn(process.execPath, [
      ...
      // The source names which host's stop hook triggered this pass. It was fixed at
      // `claude-stop` while three hosts ran the same configuration, which made every unattended
      // pass look like Claude's.
      '--source', `${currentAgentIdentity().agent}-stop`, '--quiet',
    ], {
      cwd: root,
      detached: true,
      stdio: 'ignore',
