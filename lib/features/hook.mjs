...
export async function runPreToolHook({ root, input = process.stdin, output = process.stdout } = {}) {
  ...
  try {
    ...
    const consumed = resolveReadMemoryTopic(hookInput, resolvedRoot, resolveMemoryDir(resolvedRoot));
    if (consumed) {
      const request = {
...
async function runServer(root) {
  // The daemon and the CLI have to answer from the same corpus and the same snapshot, so both take
  // their directories from the one resolver rather than from a private copy of the rule.
...
export async function runMemoryHookCli(rawArgs = process.argv.slice(2)) {
  ...
  // The marker file exists only while disabled; naming it on enable pointed at a path just removed.
  process.stdout.write(enabled
    ? 'memory hook enabled\n'
    : `memory hook disabled (${path.relative(options.root, result.marker)})\n`);
  return 0;
}
...
if (isMemoryCliEntry(import.meta.url)) {
  Promise.resolve(runMemoryHookCli()).then((code) => {
    ...
    process.stderr.write(`ownmem hook: ${error.message}\n`);
    process.exitCode = 1;
  });
}
