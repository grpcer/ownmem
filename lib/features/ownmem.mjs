...
// Same layer as the report and for the same reason: it reads the event ledger and the three
// ledgers the report reads, and reduces one day of them to a package committed to Git.
async function archive(args) {
  const module = await importLayerModule('./archive.mjs', 'dashboard', 'telemetry archive');
  return module.runCli(withProjectPaths(args));
}

// The unattended pass that calls the archive above and then reads what is already in Git. Same
// layer, because everything it inspects -- packages, the fleet view, the locks -- lives there.
...
Agent-facing commands:
  ...
  ownmem compile
  ownmem report --since 7d [--governance] [--fleet]
...
Options:
  ...
  --json                           Emit an ownmem-query-result/v5 envelope instead of the human summary
...
Options:
  ...
  archive: 'Usage: ownmem archive [--day YYYY-MM-DD] [--backfill]\n\n  Reduce a UTC day of local telemetry to a counted package under <memory-dir>/telemetry/, committed\n  to Git. Event files expire after thirty days and never leave the machine that wrote them; the\n  package is the durable, portable record. Counts only -- no query text, digest, trace id, path,\n  machine or account name, or timestamp finer than the day. Idempotent, and a day with nothing new\n  is not rewritten. The default sweep stops before today; --day archives today as partial.',
  daily: `Usage: ownmem daily [options]

  The unattended daily pass, wired to session start, session stop and post-commit. Archives every
  finished day, commits the packages, inspects what is in Git, and prints only what is wrong --
  its output lands in a model's context, so silence is the success case. Runs real work at most
  once per UTC day and always exits 0.
...
Options:
  ...
  --no-fetch         Skip the network reach behind the ahead/behind counts
  ...
  report: 'Usage: ownmem report [--since 7d] [--governance] [--fleet]\n\n  Summarise local recall telemetry. Reads .local-test/ only.\n  --governance prints the review material waiting for a person instead of the summary; it reads and prints, and every item it prints still needs somebody to merge it.\n  --fleet reads the committed daily packages from every machine instead of this one\'s expiring event files, leads with which machine covered which day, and names the days that have commits but no package.',
