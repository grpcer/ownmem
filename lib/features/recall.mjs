...
const FEEDBACK_VERDICTS = ['correct', 'wrong', 'retrieval_miss', 'coverage_gap', 'stale', 'conflict'];
...
const FEEDBACK_EXPECTED_FORBIDDEN = new Set(['coverage_gap']);
...
Options:
  ...
  --feedback VERDICT     record correct, wrong, retrieval_miss, coverage_gap, stale or conflict
...
function recordCliFeedback(options, searches, warnings) {
  ...
  if (options.feedback === 'correct' && envelope.results.length === 0) {
    throw new Error('--feedback correct requires a trusted top result; use retrieval_miss with --expected when recall returned nothing');
  }
