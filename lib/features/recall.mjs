...
function parseArgs(rawArgs) {
  const options = {
    ...
    memoryDir: process.env.OWNMEM_DIR || process.env.ORIVEO_MEMORY_DIR || null,
    indexDirectory: null,
    limit: 3,
    tier: 'default',
    excludeMemoryIds: [],
  ...
  for (let index = 0; index < rawArgs.length; index += 1) {
    ...
    // usage() has always shown `-- <query>`, but the separator itself fell through to the unknown
    // option branch, so the documented invocation could not be typed.
    if (passthrough) {
  ...
  // Resolved after the loop so that --root is final. The historical .claude/memory default pointed a
  // perfectly healthy .ownmem or .memory installation at an empty directory, and the snapshot has to
  // be read from where the compiler publishes it.
...
function usage() {
  return `Usage: ownmem recall --runtime [options] -- <keyword|path|natural-language question> [...]

The snapshot runtime lane. \`ownmem recall\` answers from Markdown by default and switches here as
soon as --runtime, --stdio, or any other option below is present; the project's root, memory
directory and index directory are pinned for you. Reads the compiled snapshot and runs a
deterministic recall. An unusable snapshot is rebuilt, and only a failed rebuild falls back to
Markdown BM25F.
...
Options:
  ...
  --index-dir PATH       local snapshot directory (default <memory-dir>/index)
...
Examples (prefix with \`npx\` when ownmem is not on PATH):
  ownmem recall --runtime -- notifyContentDidChange
  ownmem recall --runtime -- 'history shows an empty page right after login'
  ownmem recall --multi --runtime -- 'body text is clipped on mobile' 'implicit grid track overflow' 'grid-cols-1'
  ownmem recall --stdio
  ownmem recall --runtime --feedback correct -- 'never stash in a shared worktree'
  ownmem recall --runtime --feedback miss --expected feedback_no_stash -- 'do not tuck my changes away'`;
}
...
export async function runStdio(options, { input = process.stdin, output = process.stdout } = {}) {
  ...
  if (runtime.warnings.length > 0) {
    process.stderr.write(`ownmem recall: local observability skipped: ${[...new Set(runtime.warnings)].join('; ')}\n`);
  }
  ...
  for await (const line of lines) {
    ...
    try {
      ...
      if (queried.warning) process.stderr.write(`ownmem recall: local observability skipped: ${queried.warning}\n`);
...
export async function runCli(rawArgs = process.argv.slice(2), dependencies = {}) {
  ...
  if (observabilityWarnings.length > 0) {
    process.stderr.write(`ownmem recall: local observability skipped: ${[...new Set(observabilityWarnings)].join('; ')}\n`);
  }
  return 0;
}
...
if (isMemoryCliEntry(import.meta.url)) {
  Promise.resolve().then(() => runCli()).then((code) => {
    ...
    process.stderr.write(`ownmem recall: ${error.message}\n`);
    process.exitCode = 1;
  });
}
