...
import { MEMORY_CONTEXT_BUDGETS, memoryEnvelopeHandedOver } from '../memory-runtime-contract.mjs';
...
export function parseRecallOptions(rawArgs) {
  ...
  for (let index = 0; index < rawArgs.length; index += 1) {
    ...
    if (argument === '--') {
      passthrough = true;
    } else if (!passthrough && ['--root', '--memory-dir', '--index-dir', '--embedding-dir', '--limit', '--tier', '--exclude-memory-id', '--feedback', '--expected', '--feedback-file', '--usage-file', '--observability-dir', '--surface'].includes(argument)) {
  ...
  // `claude-hook` predates the surface/agent split and is still accepted; who was asking is the
  // orthogonal `agent` field, resolved from the host's declaration or its environment.
  if (!['cli', 'hook', 'claude-hook'].includes(options.surface)) throw new Error('--surface must be cli, hook or claude-hook');
  options.memoryDir = resolveMemoryDir(options.root, options.memoryDir);
  options.indexDirectory ??= memoryIndexDir(options.memoryDir);
  options.consumptionEligible = options.stdio ? options.surface !== 'cli' : true;
...
Options:
  --json                 one query emits ownmem-query-result/v6; several emit a batch wrapper
...
function resultScore(result) {
  // A pointer has no normalized score -- nothing ranked it against a top result -- so its gate
  // score stands in. Both are in 0..1 and both answer the only question the ledger asks of this
  // number: how sure was the runtime about this entry.
  return Number(result?.score ?? result?.gate_score ?? 0);
}
...
function emptyHitLine(abstain) {
  ...
  // below-content-threshold never reaches here: that envelope carries pointers, and printHumanSearch
  // prints those instead of an empty-hit line. Reaching this function with it would mean the tier
  // and the pointer list disagreed, which the protocol schema refuses outright.
...
/**
 * Pointers, printed as what they are: places to look, with no quote and no claim.
 *
 * The wording carries the tier. "closest memories" and "read them to decide" tell the reader the
 * runtime did not vouch for these, which is the whole content of the middle tier -- printing them
 * in the same shape as a quoted hit would undo the distinction the envelope just made.
 */
function printPointers(envelope, { alongsideContent }) {
  if (envelope.pointers.length === 0) return;
  process.stdout.write(alongsideContent
    ? `  also qualified (${envelope.delivery.eligible} candidate(s) in all; not quoted):\n`
    : '  closest memories (not quoted -- below the confidence needed to answer; read them to decide):\n');
  for (const pointer of envelope.pointers) {
    process.stdout.write(`      ${pointer.memory_id}  [gate=${pointer.gate_score.toFixed(3)}]\n`);
    process.stdout.write(`          ${pointer.summary}\n`);
    process.stdout.write(`          file ${pointer.path}\n`);
  }
}
...
function printHumanSearch(query, envelope) {
  ...
  if (envelope.results.length === 0) {
    if (envelope.delivery.tier === 'pointers') {
      printPointers(envelope, { alongsideContent: false });
      process.stdout.write('\n');
      // Counted as zero hits on purpose. A pointer is not an answer, and the caller's hit count
      // feeds the ledger that decides whether recall delivered anything.
      return 0;
    }
  ...
  printPointers(envelope, { alongsideContent: true });
  process.stdout.write('\n');
...
function recordCliDelivery(options, searches, warnings) {
  ...
  for (const { envelope, observationTraceId } of searches) {
    // Pointers are delivered too. This ledger is what a later full-text open pairs against, and a
    // pointer is the case where the caller is most likely to open the file -- telling them where to
    // look is the entire content of the tier. Skipping it would make exactly those reads
    // unattributable, and an outcome receipt for one would be refused for want of a trace.
    const handedOver = memoryEnvelopeHandedOver(envelope);
    if (!observationTraceId || handedOver.length === 0) continue;
    const delivery = recordMemoryDelivery({
      ...
      topics: handedOver.map((entry) => entry.memory_id),
      surface: 'cli',
    ...
    rememberMemoryRecall({
      ...
      sessionId: null,
      snapshotId: envelope.source.snapshot_id,
      returnedTopics: handedOver.map((entry) => entry.memory_id),
      source: 'cli',
    });
  }
}
...
function recordCliUsage(options, searches) {
  ...
  return searches.map(({ usageQuery, envelope }) => recordUsage({
    ...
    query: usageQuery,
    results: memoryEnvelopeHandedOver(envelope),
  }));
}
...
function recordCliFeedback(options, searches, warnings) {
  ...
  // Pointers count as something coming back. A tier that names three memories has not returned
  // nothing, and letting a caller file that as a correct abstention would put "the corpus had no
  // answer" in the ledger for a query the runtime answered with three candidates -- which is the
  // one verdict nothing else can correct, because it asserts an absence.
  const handedOver = memoryEnvelopeHandedOver(envelope);
  if (options.feedback === 'correct' && handedOver.length === 0) {
  ...
  if (options.feedback === 'correct_abstain' && handedOver.length > 0) {
    throw new Error(`--feedback correct_abstain says recall was right to return nothing, but it returned ${handedOver.map((entry) => entry.memory_id).join(', ')}; use correct when the top result is right, or wrong when it is not`);
  }
  ...
  const returned = memoryEnvelopeHandedOver(envelope).map((entry) => entry.memory_id);
...
function writeCliHuman(runtime, options, searches, feedback, usageEntries) {
  ...
  if (!options.feedback) {
    ...
    if (abstained) {
      ...
      // The other outcome, which had no offered action at all -- and which the numbers say is the
      // likelier one on this surface. `retrieval_miss` requires naming the memory that should have
      // come back, so a reader whose honest report is "none of these, and I do not think anybody
      // wrote this down" structurally cannot use it; the fitting verdict is `coverage_gap`, which
      // must NOT carry an expected memory. Measured 2026-09-20: of the 65 pointer deliveries this
      // machine has actually produced, every one of the 14 that later received a verdict was a
      // `coverage_gap`, none was a `correct`; and replaying the ledger, 83 of 187 pointer deliveries
      // come from queries filed as `wrong`, where no right answer exists by definition. Leaving that
      // outcome unnamed is how the queue stays quiet about the queries nobody has written about.
      process.stdout.write(`None of these, and nothing is written down about it? Log that instead: npx ownmem recall --feedback coverage_gap -- '${sample}'\n`);
    }
  }
