...
import {
  ...
  percentile,
  ratio,
  summarizeRetrievalBySurface,
  summarizeRetrievalBySurfaceAgent,
} from '../memory-metrics.mjs';


// Re-exported because it was part of this module's public surface before the counting rules moved
// into memory-metrics.mjs, and a helper changing address is not a reason to break an importer.
export { percentile };
...
function parseArgs(args) {
  const options = {
    ...
    json: false,
    full: false,
    // The north star replays the recorded `wrong` queries against the live index, which is roughly a
    // second of work on this repository's ledger (106 distinct queries, ~8ms each in process). That
    // is fine for a command somebody typed and wrong for a caller on a hot path, so it is a flag
    // rather than a constant: the CLI replays, and the Stop-hook session report and the dashboard
    // server -- both of which call collectMemoryReport directly -- do not.
    replay: true,
  };
  for (let index = 0; index < args.length; index += 1) {
    ...
    else if (argument === '--no-replay') options.replay = false;
...
Options:
  ...
  --full                  Also print cohort splits, abstain reasons, token distribution and build
                          counters. The default is the four lines that change what you do next
  ...
  --no-replay             Skip the north-star replay of the recorded \`wrong\` queries. The line then
                          says it was not measured rather than printing a stale number
...
function retiredGovernanceSurface(now) {
  const block = stream => ({
    stream,
    connected: false,
    reason_code: 'collector-not-wired',
    reason: 'promotion governance was retired; this surface is not collected',
  });
  return {
    schema: 'ownmem-governance-surface/v1',
    generated_at: now.toISOString(),
    review_material: block('review_material'),
    observation_window: block('observation_window'),
    quarantine: block('quarantine'),
    quota_settlement: block('quota_settlement'),
  };
}
...
/**
 * Which observation window this installation is in, and whether the ranking still matches it.
 *
 * Every number below is a number about a ranking. Printed without saying which one, a report read
 * across a ranking change compares two different systems under one heading -- which is the failure
 * windows exist to make visible. An installation that has frozen nothing says so rather than
 * implying stability it never claimed.
 */
function collectProtocolWindow() {
  return {
    window_id: null,
    opened_day: null,
    frozen_ranking_hash: null,
    installed_ranking_hash: null,
    matches: null,
    changed_files: [],
    registry_valid: true,
  };
...
function staticGateHealth(root, memoryDir, health) {
  ...
  // Topic count only. The byte ceiling was removed in 2.0 -- it had been raised by hand every time
  // it bound, so it never refused anything -- and a lock that no longer declares one must not read
  // as unhealthy for the omission.
  ...
  return {
    ...
    quota: {
      ...
      // Still reported, because corpus size is worth watching; no longer a ceiling.
...
function recommendations(report) {
  ...
  if (report.quality.feedback.total > 0 && report.quality.feedback.wrong_miss_rate > 0.1) {
    ...
    // The residual is appended when it was actually replayed, because it answers the one thing the
    // windowed verdict counts cannot: how much of what was reported is still true today. Without it
    // this recommendation fires on a backlog a later improvement may already have emptied.
    const residual = report.quality.false_delivery.status === MEMORY_EVALUATION_MEASURED
      ? `; ${report.quality.false_delivery.still_quoted}/${report.quality.false_delivery.distinct_queries} distinct wrong receipt(s) are still quoted today`
      : '';
    add('review-feedback', 70, 'high', 'medium', `${shape.returned} wrong delivery / ${shape.abstained} wrong abstention / ${shape.unknown} legacy-unknown / ${report.quality.feedback.verdicts.retrieval_miss} retrieval_miss among ${report.quality.feedback.total} receipt(s) (resolved ones not deducted here)${residual}`, 'Review wrong deliveries as precision failures and wrong abstentions as missing-retrieval or corpus failures; then run memory-feedback-review to re-run each named miss and drop the ones a later improvement already fixed.');
  }
...
/**
 * Re-runs one recorded query through the real retrieval runtime, in process.
 *
 * The runtime is built once and reused, which is what makes replaying a hundred queries affordable:
 * measured 2026-09-19 on this repository's ledger, 106 distinct `wrong` queries take 845ms in total,
 * against roughly 300ms each if every one paid for its own process the way memory-feedback-review
 * does (it spawns per query because it also writes receipts and wants the production entry point).
 *
 * Observability is off on purpose. These are replays of questions somebody already asked, and letting
 * them write recall events would inflate every count in the report that is measuring them.
 */
export function createFalseDeliveryReplay({ root, memoryDir }) {
  let runtime;
  try {
    runtime = createMemoryRecallRuntime({ root, memoryDir, observability: false }, { observer: null });
  } catch {
    return null;
  }
  return (query) => {
    try {
      const result = queryMemoryRuntime(runtime, query, { limit: 3 });
...
export function collectMemoryReport({
  ...
  // Default false: importers on a hot path (the Stop-hook session report, the dashboard server) get
  // the honest "not measured" rather than a second of retrieval per call. The CLI opts in.
  replayFalseDelivery = false,
  feedbackFile: suppliedFeedbackFile = null,
} = {}) {
  ...
  const consumptionSources = [...new Set(consumed.map(event => event.payload.via || 'hook'))].sort();
  ...
  // The pointers tier, which the funnel above cannot see at all: it counts only recalls that were
  // NOT abstentions, and a pointers envelope sets abstained=true with reason below-content-threshold.
  // So 59 of the 284 current-stack answers in the 30-day window were invisible to every delivery
  // number in this report.
  //
  // Worth its own funnel now rather than later: replaying the 79 distinct `correct` receipts against
  // the current index, only 36 still arrive as prose. "The right answer arrives as a pointer" is the
  // normal case, so whether anybody opens a pointer is no longer a footnote -- it is the question of
  // whether the middle tier does any work at all. First reading: 59 pointer deliveries, 0 opened.
  //
  // The tier is reconstructed from abstained + abstain_reason rather than from a new event field on
  // purpose. Those two are already recorded and already contractual, and adding a field would put
  // every row written before today outside the denominator.
  const pointerRecalls = currentRuntimeRecalls.filter(event => (
    event.payload.abstained === true
    && event.payload.abstain_reason === 'below-content-threshold'
    && event.payload.returned_topics.length > 0
  ));
  const pointerDeliveredTraceIds = new Set(pointerRecalls
    .filter(event => deliveredByTrace.has(event.trace_id))
    .map(event => event.trace_id));
  const pointerOpenTraceIds = new Set(consumed
    .filter(event => pointerDeliveredTraceIds.has(event.trace_id))
    .map(event => event.trace_id));
  ...
  const observableEligible = eligible.filter(event => (
    ...
    && ['cli', 'hook', 'claude-hook'].includes(event.payload.surface)
  ));
  ...
  // The north star. Read off the file-backed feedback ledger rather than the windowed event stream,
  // and deliberately so: the question is "does the engine still get the things it was told it got
  // wrong", which is a property of the whole history, not of the last seven days. A residual that
  // shrank because old complaints fell out of a window would be a measurement artefact.
  const feedbackFile = suppliedFeedbackFile
    ? path.resolve(root, suppliedFeedbackFile)
    : path.resolve(root, DEFAULT_FEEDBACK_FILE);
  const feedbackInbox = readFeedbackInbox(feedbackFile);
  const falseDelivery = summarizeKnownFalseDelivery({
    inbox: feedbackInbox,
    // A function is accepted as well as a boolean so a test can drive the residual without building a
    // retrieval runtime. Without it the only way to gate this number would be to assert the shape of
    // the object, which is the kind of test that stays green while the arithmetic is wrong.
    replay: typeof replayFalseDelivery === 'function'
      ? replayFalseDelivery
      : replayFalseDelivery ? createFalseDeliveryReplay({ root, memoryDir }) : null,
  });
  ...
  // The feedback ledger's own unreadable rows, which no command a consumer has was printing. It is
  // the north star's denominator: rows the reader rejects are silently absent from it, so a ledger
  // written under an older schema reads as "nobody ever reported a wrong answer" rather than as
  // "your history is no longer being read". Measured 2026-09-20 on a real repository upgraded from
  // 0.6.0, whose ledger carried an ownmem-recall-feedback/v1 row from an even earlier version: the
  // residual reported 0 distinct receipts and nothing said why. The remedy sentence was already in
  // the reader; it just had no surface.
  for (const error of feedbackInbox.errors.slice(0, 1)) {
    gap('invalid-feedback', `${feedbackInbox.errors.length} row(s) of`
      + ` ${path.relative(root, feedbackFile)} could not be read, so they are absent from the north star`
      + ` and from every verdict count. First: ${error}`);
  }
  if (healthError) gap('health-read-failed', `Static health read failed: ${healthError}`);

  // Named `protocol_window` rather than `window`, which this report already uses for the time range
  // it covers. Two different windows with one name is how a reader ends up comparing numbers across
  // a ranking change while believing the opposite.
  const protocolWindow = collectProtocolWindow();

  const report = {
    schema: 'ownmem-report/v9',
    generated_at: now.toISOString(),
    environment: {
      installation_id: memoryInstallationId({ root, directory }),
      event_data_local_only: true,
    },
    protocol_window: protocolWindow,
    window: {
    ...
    metric_roles: {
      // Changed 2026-09-19. The previous north star was the confirmed full-text-open rate with
      // actual application printed beside it; actual application is now declared a permanent blind
      // spot (see summarizeKnownFalseDelivery for the 547-turn read that settled it), and an open
      // rate the envelope is designed to keep low was never an outcome. The residual is: both halves
      // of it are already-recorded facts, and it goes down only when retrieval actually improves.
      north_star: 'known_false_delivery_residual_rate',
      north_star_event: 'feedback.recorded',
    ...
    delivery: {
      ...
      // The pointers tier's own funnel, kept apart from the three counters above rather than folded
      // into them: those describe quoted answers, and blending a tier that explicitly does not vouch
      // for anything into a delivery rate would erase the distinction the tiers exist to draw.
      pointer_traces: pointerRecalls.length,
      pointer_delivered_traces: pointerDeliveredTraceIds.size,
      pointer_open_traces: pointerOpenTraceIds.size,
      pointer_open_rate: ratio(pointerOpenTraceIds.size, pointerDeliveredTraceIds.size),
    ...
    // Only boolean cache_hit values prove an observable cache surface; 0/0 remains null, not 0%.
      ...
      retrieval_by_surface_agent: summarizeRetrievalBySurfaceAgent(retrievalAttempts),
    ...
    quality: {
      ...
      // Not a fourth stream: it is stream 1 re-read. The verdicts above count what people reported;
      // this replays the `wrong` ones against the live index and reports how much of it is still
      // true. Same rows, different question -- "what did they tell us" versus "what is still broken".
      false_delivery: falseDelivery,
    ...
    governance: retiredGovernanceSurface(now),
...
//
// The no-sample branch now says it expects to stay that way, which is a finding rather than a mood.
// 547 real user turns across 75 local transcripts were read one at a time looking for anything that
// reads as "that memory was right": zero. The user evaluates the deliverable, not the memory, and
// has no reason to know which memory was injected -- so this numerator is not waiting on a lower
// confirmation bar, it is waiting on information nobody has. The one honest path to a sample is
// `--confirmed-by host`: a gate or a test confirming the memory's own assertion. Saying so here is
// what stops the next reader from trying to fix the collection again.
function formatActualApplication(outcome) {
  ...
  if (outcome.total === 0) {
    return 'actual application has no sample yet (0 outcome receipt(s); only a user or the host can confirm one, so receipts do not accumulate on their own)'
      + ', and is expected to stay that way: a user judges the deliverable, not which memory was injected, so this confirmation is not something a lower bar can buy. The one path to a sample is --confirmed-by host, a gate or test confirming the memory\'s own assertion';
  }
...
// The north star, in one clause, with its denominator. Never a percentage over something that was not
// replayed: a residual printed from a stale review would read exactly like a measured improvement.
function formatFalseDeliveryResidual(falseDelivery) {
  if (falseDelivery.status !== MEMORY_EVALUATION_MEASURED) {
    // Not measured is stated as not measured. A residual has no meaningful zero: printing 0% for a
    // replay that never ran would read exactly like the best possible result.
    return `not measured — ${falseDelivery.unmeasured_reason}`
      + ` (${falseDelivery.distinct_queries} distinct \`wrong\` receipt(s) on the ledger)`;
  }
  const replayed = falseDelivery.still_quoted + falseDelivery.no_longer_quoted;
  return `${falseDelivery.still_quoted} of ${replayed} distinct \`wrong\` receipt(s) are still answered with prose`
    + ` (${percentage(falseDelivery.residual_rate)})`
    + `${falseDelivery.unreplayable > 0 ? `; ${falseDelivery.unreplayable} could not be replayed and are excluded` : ''}`
    + '. Replayed against the index on disk now, over the whole ledger rather than the window above,'
    + ' so it falls only when retrieval actually improves';
}
...
// The second dimension, printed beside the first rather than instead of it. The excluded counts
// are always shown when non-zero: a reader who sees only the attributable buckets would otherwise
// have no way to tell how much of the window they describe.
function formatRetrievalBySurfaceAgent(breakdown) {
  if (!breakdown) return '';
  const entries = Object.entries(breakdown.buckets || {});
  const parts = entries
    .map(([key, entry]) => `${key} ${percentage(entry.abstain_rate)} (${entry.abstained}/${entry.attempts})`);
  const notes = [];
  if (breakdown.legacy_attempts > 0) {
    notes.push(`legacy ${breakdown.legacy_attempts} attempt(s) unattributed`
      + ' (written before agent attribution; those days were genuinely mixed and are counted in no agent)');
  }
  if (breakdown.nested_attempts > 0) {
    notes.push(`nested ${breakdown.nested_attempts} attempt(s) excluded (more than one host's markers present)`);
  }
  if (parts.length === 0 && notes.length === 0) return '';
  const detail = [parts.join(', '), ...notes].filter(Boolean).join('; ');
  return ` [by surface x agent: ${detail}]`;
}
...
/**
 * The default report answers four questions and stops; `--full` prints everything.
 *
 * Every line below is worth having, but not on every run, and the cost of printing them together is
 * not neutral: a wall of long lines is read once and skipped afterwards, which is the same failure
 * the per-turn panel had. The four kept by default are the ones that change what somebody does
 * next -- is it being used, is it fast enough, is it right, and what should I do about it. Cohort
 * splits, abstain-reason breakdowns, token distributions and build counters are diagnosis: you go
 * looking for them once one of the four says something is wrong.
 */
export function formatMemoryReport(report, { full = false } = {}) {
  const observedWindow = report.window.observed_from
    ...
    : 'no local events';
  const lines = [
    // "stack" is the recall runtime's own version, and it is on this line rather than buried in the
    // north star because it scopes every number below it: a report that mixed two stacks would be
    // comparing two different rankings without saying so.
    `Memory report (${report.window.requested}; install ${report.environment.installation_id || 'uninitialised'}; observed ${observedWindow}; recall stack ${report.delivery.runtime_version}, which is the version every number below was produced by)`,
    `North star (known false-delivery residual): ${formatFalseDeliveryResidual(report.quality.false_delivery)}`,
    // Kept directly under it, because the residual says nothing about volume and a reader who only
    // saw the residual could not tell one recall from a thousand. The open rate is a coverage floor,
    // not an outcome: the 400-token envelope is built so the caller rarely needs the source.
    `Delivery (current stack): produced ${report.delivery.produced_traces} -> delivered ${report.delivery.delivered_traces} (${percentage(report.delivery.delivery_rate)}) -> confirmed full-text opens ${report.delivery.fulltext_open_traces} (${percentage(report.delivery.fulltext_open_rate)}); pointers tier ${report.delivery.pointer_traces} answer(s), ${report.delivery.pointer_delivered_traces} recorded, ${report.delivery.pointer_open_traces} opened (${percentage(report.delivery.pointer_open_rate)}) -- a pointer is only a place to look, so opening it is the whole point rather than a fallback`,
    // Directly under the north star, and above every rate below it: the numbers here describe one
    // ranking, and which one is not something a reader can recover afterwards. The north star keeps
    // the first line -- it is the one metric this report exists to publish.
    ...
    ...(full ? [`Whole window: ${report.samples.recalls} recall(s), overall P95=${report.performance.total_ms.p95 ?? 'n/a'}ms; ${formatEmbeddingPerformance(report.performance.embedding)}, cache hit ${report.performance.cache_hit_observed === 0 ? 'unavailable (production recall has no result cache, so it cannot be measured)' : `${percentage(report.performance.cache_hit_rate)} (observable ${report.performance.cache_hit_observed}/${report.samples.recalls})`}, retrieval abstain ${percentage(report.performance.retrieval_abstain_rate)} (${report.performance.retrieval_abstained}/${report.performance.retrieval_attempts})${formatRetrievalBySurface(report.performance.retrieval_by_surface)}${formatRetrievalBySurfaceAgent(report.performance.retrieval_by_surface_agent)}, session dedup ${report.performance.deduplicated} (${percentage(report.performance.deduplication_rate)})${formatAbstainReasons(report.performance.abstain_reasons)}${formatSnapshotDegradation(report.performance.snapshot_degradation)}`] : []),
  ...
  // A data gap is never hidden: it says the numbers above are computed over a subset, which changes
  // how every one of them should be read.
  ...
  if (!full) lines.push('Run with --full for cohort splits, abstain reasons, token distribution and build counters.');
  return `${lines.join('\n')}\n`;
}
...
export function runCli(rawArgs = process.argv.slice(2)) {
  ...
  // The CLI is the caller that may spend a second on the north star, so it is the one that opts in.
  // collectMemoryReport's own default stays off, which is what keeps the Stop-hook session report and
  // the dashboard server -- both direct importers -- off the replay path.
  const report = collectMemoryReport({ ...options, replayFalseDelivery: options.replay });
  process.stdout.write(options.json ? `${JSON.stringify(report, null, 2)}\n` : formatMemoryReport(report, { full: options.full }));
  return 0;
}
