...
function recommendations(report) {
  ...
  if (report.quality.feedback.total > 0 && report.quality.feedback.wrong_miss_rate > 0.1) {
    add('review-feedback', 70, 'high', 'medium', `${report.quality.feedback.verdicts.wrong + report.quality.feedback.verdicts.retrieval_miss}/${report.quality.feedback.total} logged wrong or retrieval_miss (resolved ones not deducted here)`, 'Run memory-feedback-review: it re-runs each recall and drops the ones a later improvement already fixed, leaving only what still needs triggers, samples or ranking rules.');
  }
...
export function collectMemoryReport({
  ...
  const feedbackSummary = summarizeFeedback(eventInbox(feedback, ['verdict']));
...
function formatQualityStreams(quality) {
  ...
  return [
    `Quality / retrieval verdicts (n=${quality.feedback.total} recorded receipts): ${verdicts}; offline Recall@1/Recall@5/MRR are process gates only; accuracy is not reported — ${quality.feedback.accuracy_unmeasurable_reason}`,
