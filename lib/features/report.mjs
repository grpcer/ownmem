...
function parseArgs(args) {
  ...
  // Resolved after the loop so that --root is final; the .claude/memory default reported "static
  // health read failed" on an installation whose corpus was right there.
...
if (isMemoryCliEntry(import.meta.url)) {
  try {
    ...
    process.stderr.write(`ownmem report: ${error.message}\n`);
    process.exitCode = 1;
  }
}
