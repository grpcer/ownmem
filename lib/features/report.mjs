...
import { utcDay } from '../memory-archive.mjs';
import { collectFleetReport, formatFleetReport } from '../memory-fleet.mjs';
...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    else if (argument === '--fleet') options.fleet = true;
...
Options:
  ...
  --fleet                 Read the committed daily telemetry packages from every machine instead of
                          this machine's expiring event files. Prints which machine covered which
                          day first, and names the days that have commits but no package.`;
}
...
export function runCli(rawArgs = process.argv.slice(2)) {
  ...
  if (options.fleet) {
    // A different question and a different source: the packages in Git rather than this machine's
    // .local-test/. It gets its own schema for that reason -- forcing both shapes through the
    // report object would make every field of each one optional to the other.
    const now = new Date();
    const fleet = collectFleetReport({
      root: options.root,
      memoryDir: options.memoryDir,
      fromDay: utcDay(new Date(now.getTime() - parseDuration(options.since))),
      toDay: utcDay(now),
      requested: options.since,
    });
    process.stdout.write(options.json ? `${JSON.stringify(fleet, null, 2)}\n` : formatFleetReport(fleet));
    return 0;
  }
