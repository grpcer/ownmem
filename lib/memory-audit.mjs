...
import { collectMemoryTrustAudit, formatMemoryTrustSummary } from './memory-trust-audit.mjs';
...
function inspectQuota({ file, activeTopicFiles, hookCounts, writeLock }) {
  ...
  if (!['ownmem.quota/v2', 'ownmem.quota/v3'].includes(lock.schema)) return [issue('error', 'quota-schema', 'Memory quota lock uses an unsupported schema.')];
  const output = [];
  ...
  if (writeLock) {
    if (activeCount > maxActive) return [issue('error', 'quota-ratchet-raise', 'Ratchet update would raise a locked quota; merge or archive first.')];
    lock.max_active_l3 = activeCount;
    // Dropped rather than re-frozen: a byte ceiling that gets raised by hand every time it binds is
    // not a quota. The topic count is the ratchet; see the lock's own rationale.
    delete lock.max_active_bytes;
...
export function collectMemoryAudit({ root, memoryDir = '.ownmem', writeLock = false, strictWorkingTree = false } = {}) {
  ...
  const trust = collectMemoryTrustAudit({ root, memoryDir, strictWorkingTree });
  output.push(...trust.issues);
  return {
    ...
    summary: {
      l1: 1,
      l2: l2.files.length,
      active_l3: activeTopicFiles.length,
      archived: recursiveMarkdownCount(path.join(memoryDirectory, 'archive')),
      trust: trust.summary,
      quota_utility: trust.quota_utility,
    },
  };
}
...
export function formatMemoryAudit(report) {
  ...
  // The trust and evidence tiers were only ever printed by `ownmem trust check`, so anyone running
  // the audit alone saw the quarantine errors without the counts that put them in proportion.
  if (report.summary.trust) output.push(...formatMemoryTrustSummary(report.summary.trust, report.summary.quota_utility));
...
export function auditPlatformContract(platform) {
  ...
  return {
    ...
    argv: [fileURLToPath(new URL('../bin/ownmem.mjs', import.meta.url)), 'audit'],
  };
}
