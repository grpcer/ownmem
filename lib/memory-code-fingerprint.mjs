...
// Process-local memo for file anchors, bounded and keyed by inode identity.
//
// A single recall resolves the receipts of every relevant candidate, and a resident runtime does it
// again for every query in the session. Without this, the same symbol is read and re-sliced each
// time: measured 2026-09-18, reading and fingerprinting anchors was 40% of a hot recall.
//
// The key is (inode, size, mtime, ctime) at nanosecond resolution. That is a *cache invalidation*
// signal and deliberately not a staleness signal -- invariant 2 above still holds, because a
// fingerprint is never declared unchanged on the strength of a timestamp. When any of the four
// differs the slice is recomputed from the file's bytes, and the fingerprint that decides drift is
// still the hash of those bytes. ctime is in the key because it moves on every inode change
// including one that restores mtime, so the one shape that could defeat an mtime-only key -- an
// equal-size rewrite with the timestamp put back -- does not survive it.
//
// Directory anchors are deliberately not memoised: a directory's own stat does not move when a
// member's content changes, so there is no identity here that would expire the entry.
const FINGERPRINT_MEMO_LIMIT = 4096;
const fingerprintMemo = new Map();

function fileIdentity(stats) {
  return `${stats.ino}\0${stats.size}\0${stats.mtimeNs}\0${stats.ctimeNs}`;
}

function memoised(key, identity, compute) {
  if (!identity) return compute();
  const cached = fingerprintMemo.get(key);
  if (cached && cached.identity === identity) return cached.value;
  const value = compute();
  // Bounded by insertion order: a long-lived runtime walking a large repository must not grow this
  // map without limit. Map preserves insertion order, so the oldest key is the first one.
  if (fingerprintMemo.size >= FINGERPRINT_MEMO_LIMIT) {
    fingerprintMemo.delete(fingerprintMemo.keys().next().value);
  }
  fingerprintMemo.set(key, { identity, value });
  return value;
}

// Exported for tests that need to prove a recomputation happened rather than a cache hit.
export function resetCodeFingerprintMemo() {
  ...
  const visit = (directory, prefix) => {
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        // A nested checkout (linked worktree, submodule, clone) is not part of this anchor: its
        // files belong to another commit and disappear with it.
        if (existsSync(path.join(next, '.git'))) continue;
        visit(next, member);
...
export function fingerprintCodeAnchor(root, anchor, { listDirectoryMembers = defaultDirectoryMembers } = {}) {
  ...
  // One stat answers all three questions this function opens with -- does the path resolve, is it a
  // directory, and what is the file's identity for the memo below. It used to be three syscalls
  // (existsSync + statSync + the memo's own stat) on a path that runs once per relevant candidate.
  const stats = statSync(absolutePath, { bigint: true, throwIfNoEntry: false });
  if (!stats) return { status: 'missing-path', coarse: false, weakSymbol: false, fingerprint: null };
  ...
  return memoised(`${absolutePath}\0${anchor.symbol || ''}`, fileIdentity(stats), () => {
