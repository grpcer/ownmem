...
function compileRanking(scan, compilerVersion, tokenizer) {
  ...
  return {
    ...
    //
    // Arming this needs coverage evidence that means the same thing in a language that inflects as
    // in one that does not. Until then the honest state is measured-not-enforced: the number is on
    // screen every run, and answer-ablation says what it is worth.
    coverage_floor: 0,
