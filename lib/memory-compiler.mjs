...
export const MEMORY_COMPILER_VERSION = '0.6.1';
...
function scanMemorySources(root, memoryDir) {
  ...
  const qualityPath = path.join(absoluteMemoryDir, 'quality.lock.json');
  if (existsSync(qualityPath)) {
    const bytes = readFileSync(qualityPath);
    sources.push({
      path: `${normalizedMemoryDir}/quality.lock.json`,
      file: 'quality.lock.json',
      kind: 'ranking-config',
      sha256: sha256(bytes),
      bytes: bytes.byteLength,
      content: bytes.toString('utf8'),
...
function compileDigest({ compilerVersion, source, hooks, dependencyState = [] }) {
...
function compileDocuments({ scan, priorSnapshot, compilerVersion }) {
...
function compileRanking(scan, compilerVersion) {
...
function buildSnapshot({ scan, priorSnapshot, compilerVersion, now, temporaryDirectory }) {
  const documentResult = compileDocuments({ scan, priorSnapshot, compilerVersion });
  ...
  const artifacts = {
    ...
    ranking: compileRanking(scan, compilerVersion),
  };
  ...
  const manifest = {
    ...
    compatibility: {
      ...
      query_result_schema: 'ownmem-query-result/v1',
...
function compileUnlocked({ root, memoryDir, indexRoot, compilerVersion, now, force, beforePublish }) {
  ...
  if (!force && currentReady && sameCompiler && priorSnapshot.manifest.source.source_sha256 === scan.sourceSha256) {
    return {
  ...
  const reusablePrior = sameCompiler ? priorSnapshot : null;
  const temporaryDirectory = path.join(indexRoot, `.tmp-${process.pid}-${randomBytes(8).toString('hex')}`);
  try {
    const result = buildSnapshot({ scan, priorSnapshot: reusablePrior, compilerVersion, now, temporaryDirectory });
...
export function compileMemoryIndex({
  ...
  // The lock file lives inside the index directory, and taking it runs mkdir -p. With the index
  // nested under the memory directory, that created the corpus directory as a side effect: a compile
  // aimed at a directory the project does not use materialised an empty .ownmem/ on a repository
  // whose memory lives in .memory/, which flipped every later probe and silently emptied recall.
  // Refuse before anything is created, so a failed compile leaves the layout exactly as it found it.
...
export function compileMemoryTopicDelta({
  ...
  const compiled = compileDocument({ source, hooks, compilerVersion, topicSourcesById, parsedTopic });
  return {
