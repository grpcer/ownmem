...
export const MEMORY_COMPILER_VERSION = '0.10.0';
  ...
  return {
    ...
    features: {
      ...
      weights: {
        ...
        query_coverage: 0.2,
        // Zero on purpose: query_span_coverage is a gate, not a ranking signal. It answers "how
        // much of the query did this candidate explain", which separates a real answer from an
        // in-domain near-miss, but it says nothing about which of two real answers is better.
        query_span_coverage: 0,
        matched_terms: 0.15,
    ...
    // The second gate, conjunctive with the thresholds above. They ask whether the candidate scored
    // high enough; this asks whether it accounted for enough of the query. The two catch different
    // kinds of no-answer: a threshold catches a query the corpus knows nothing about, and this
    // catches a query the corpus knows the vocabulary of but not the answer to. Measured
    // 2026-08-27, the curated negatives are all of the first kind, which is why 20/20 abstention
    // there coexisted with 1/7 on the real ones.
    //
    // Zero means the gate is not armed. The feature is still computed, still published as
    // abstain.top_coverage, and answer-ablation still reports what it would be worth -- what is
    // withheld is the decision, not the measurement.
    //
    // It is not armed because it is not yet fair across languages, and shipping it would trade one
    // group of users for another. Measured 2026-08-27 at the value it would take -- 0.45, the
    // midpoint of [0.405, 0.500], the widest interval where every tuning verdict holds and the
    // holdout is fully caught -- it takes the holdout in-domain negatives from 1/7 to 7/7 and
    // answer-ablation from 29.3% to 58.7% on this repository's 355 Chinese-and-English memories.
    // On the 40-language public fixture the same floor costs the German and Polish cases outright:
    // a compound or inflected query word is a different posting from the indexed one, so a correct
    // match reads as partial coverage for reasons of morphology rather than relevance. Three
    // formulations were measured (exact postings only; every lane; every lane behind a stem-length
    // guard) and each trades one side for the other.
    //
    // Armed at the 2026-08-27 measured value. Identifier/path/error lookups, exact anchors,
    // substantial non-compact tokens, and code-backed memories are exempt in the ranker so a
    // single-token symbol query and a colloquial rewrite still deliver. Natural queries that only
    // share compact fragments with a weakly evidenced memory are the in-domain false positives
    // this gate exists to stop. The 40-language public fixture still pays a morphology cost; that
    // is scored separately, not a reason to leave the gate unarmed on this Chinese-and-English
    // production corpus.
    coverage_floor: 0.45,
...
function buildSnapshot({ scan, ranking, priorSnapshot, compilerVersion, now, temporaryDirectory, tokenizer, root, memoryDir, trustLock, gitState }) {
  ...
  const manifest = {
    ...
    compatibility: {
      ...
      query_result_schema: 'ownmem-query-result/v6',
