...
const INDEX_SCHEMA_PATH = schemaPath('index', 'manifest.schema.json');
const QUERY_RESULT_SCHEMA_PATH = schemaPath('protocol', 'query-result.schema.json');
const STDIO_REQUEST_SCHEMA_PATH = schemaPath('protocol', 'stdio-request.schema.json');
const STDIO_RESPONSE_SCHEMA_PATH = schemaPath('protocol', 'stdio-response.schema.json');
const HOOK_REQUEST_SCHEMA_PATH = schemaPath('protocol', 'hook-request.schema.json');
const HOOK_CONSUME_REQUEST_SCHEMA_PATH = schemaPath('protocol', 'hook-consume-request.schema.json');
const HOOK_RESPONSE_SCHEMA_PATH = schemaPath('protocol', 'hook-response.schema.json');
const CANDIDATE_SCHEMA_PATH = schemaPath('candidates', 'candidate.schema.json');
const ARTIFACT_SCHEMA_PATHS = {
  documents: schemaPath('index', 'documents.schema.json'),
  postings: schemaPath('index', 'postings.schema.json'),
  exact_map: schemaPath('index', 'exact-map.schema.json'),
  graph: schemaPath('index', 'graph.schema.json'),
  ranking: schemaPath('index', 'ranking.schema.json'),
};
export const MEMORY_INDEX_SCHEMA = 'ownmem-index/v1';
export const MEMORY_QUERY_RESULT_SCHEMA = 'ownmem-query-result/v5';
export const MEMORY_INDEX_READER_VERSION = 5;
...
const validateCandidateSchema = lazyValidator(CANDIDATE_SCHEMA_PATH);
...
/**
 * The gate every candidate producer has to pass.
 *
 * "Deterministic, host-assisted and model-backed extractors may only produce schema-valid
 * candidates" is a property of the writer, not a promise each producer keeps: this runs where the
 * queue is written, so a producer that has not been imagined yet is still bound by it. Throwing
 * rather than dropping is deliberate -- a malformed lead is a bug in whatever produced it, and a
 * queue that silently swallowed those would be a queue nobody could trust the size of.
 */
export function validateMemoryCandidate(value) {
  return validate(validateCandidateSchema, value, 'memory candidate');
}

// The promotion-decision gate belongs beside every other contract validator, which is here -- but
// it is implemented in memory-promotion-policy.mjs, one layer below this module, because that is
// where decisions are produced and a layer may not import upward. Re-exported rather than
// reimplemented so there is one compiled copy of the schema and one place it can be wrong.
export { validateMemoryPromotionDecision } from './memory-promotion-policy.mjs';
// Same arrangement for the procedure contracts: they are implemented in core, beside the write
// path they guard, and surfaced here so every contract validator is found in one place.
export { validateMemoryProcedure, validateMemoryReplayRecord } from './memory-procedure.mjs';

// Same placement, same reason: the receipt gate belongs with the other contract validators, and it
// is implemented in memory-promotion-receipt.mjs because that module sits a layer below this one.
export { validateMemoryPromotionReceipt } from './memory-promotion-receipt.mjs';

// One schema per artifact kind, and no second reader for any earlier encoding. An older snapshot is
// refused by the reader and rebuilt from the Markdown sources, which are the actual source of
// truth; a compiled index is a derived, discardable artifact, so keeping a second parser alive to
// avoid recompiling it buys nothing and never expires.
export function validateMemoryIndexArtifact(kind, value) {
  const validator = validateArtifactSchemas[kind];
...
export function validateMemoryQueryResult(value) {
  ...
  // v4 stopped publishing max_topics / max_tokens / estimator: the tier decides all three, so the
  // ceiling is read from the one table instead of from a number the envelope repeated to itself.
  const budget = MEMORY_CONTEXT_BUDGETS[value.budget.tier];
  if (value.budget.estimated_tokens > budget.maxTokens) {
    throw new Error(
      `memory query result is invalid: /budget/estimated_tokens exceeds the ${value.budget.tier} ceiling of ${budget.maxTokens}`,
    );
  }
  if (value.results.length > budget.maxTopics) {
    throw new Error(
      `memory query result is invalid: /results holds more topics than the ${value.budget.tier} ceiling of ${budget.maxTopics}`,
    );
  }
