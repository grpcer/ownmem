...
export function enableEmbedding({
  ...
  if (mode === 'weighted' && !report.guard.recommended_weight_safe) {
    // force acknowledges the failed gate; confirm independently acknowledges the named regression.
    if (!forced || input.confirm !== true) {
      throw Object.assign(
        actionError(409, 'ab_guard_blocked',
          report.guard.negative_not_worse
            ? 'the suggested weight regressed the golden metrics; weighted mode requires force + confirm'
            : 'negative abstain got worse; weighted mode requires force + confirm'),
        {
          guard: report.guard,
  ...
  const next = validateMemoryEmbeddingConfig({
    ...
    enable_forced: mode === 'weighted' ? forced && !report.guard.recommended_weight_safe : false,
