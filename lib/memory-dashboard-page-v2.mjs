...
function renderQuality(){const overview=state.data.overview;const benchmark=state.data.benchmark;if(!overview)return skeleton();const health=overview.health;const queue=overview.feedback_queue;let evalBody=health&&health.evaluation.available?'<div class="stats">'+metric(t('evaluation_coverage'),pct(health.evaluation.topic_coverage))+metric(t('golden'),num(health.evaluation.golden_cases))+metric(t('negative'),num(health.evaluation.negative_cases))+'</div>':empty(null,t('evaluation_unavailable'));
  let feedbackBody=queue&&queue.pending.length?'<div class="tablewrap"><table><thead><tr><th>'+esc(t('column_type'))+'</th><th>'+esc(t('column_query'))+'</th><th class="num">'+esc(t('column_expected'))+'</th><th class="num">'+esc(t('column_time'))+'</th></tr></thead><tbody>'+queue.pending.map(item=>'<tr><td>'+status(item.verdict==='miss'?t('miss'):t('wrong'),item.verdict==='miss'?'warn':'bad')+'</td><td>'+esc(item.query)+'</td><td class="num">'+esc(item.expected||t('unavailable'))+'</td><td class="num">'+esc(when(item.recorded_at))+'</td></tr>').join('')+'</tbody></table></div>':empty(null,t('feedback_empty'));
  let abBody=empty(null,t('ab_empty'));if(benchmark&&benchmark.latest){const latest=benchmark.latest;const tone=!latest.component_current?'warn':latest.guard.recommended_weight_safe?'ok':'bad';const label=!latest.component_current?t('stale_report'):latest.guard.recommended_weight_safe?t('guard_passed'):t('guard_failed');const golden=latest.metrics&&latest.metrics.golden;const negative=latest.metrics&&latest.metrics.negative;
...
+'<div class="prose">'+esc(t('maintenance_help'))+'</div>';
  ...
  return '<div class="grid">'+card(t('quota'),quotaBody,'','c6')+card(t('maintenance'),maintenanceBody,'','c6')+card(t('review_due'),reviewBody,'','')+'</div>'}
...
function abVerdict(latest,matches){if(!latest)return null;if(matches===false)return {current:false,safe:false,noGain:false,label:t('report_other_provider'),tone:'warn'};const current=Boolean(latest.component_current);const safe=Boolean(latest.guard&&latest.guard.recommended_weight_safe);const golden=latest.metrics&&latest.metrics.golden;const gain=golden&&golden.a&&golden.b?Number(golden.b.recall_at_1)-Number(golden.a.recall_at_1):null;const noGain=current&&safe&&Number.isFinite(gain)&&gain<=0;return {current,safe,noGain,label:!current?t('stale_report'):noGain?t('ab_no_gain'):safe?t('guard_passed'):t('guard_failed'),tone:!current||noGain?'warn':safe?'ok':'bad'}}
...
const notice=verdict&&verdict.noGain?'<div class="notice">'+esc(t('ab_no_gain_help'))+'</div>'
