...
// The hero is the known false-delivery residual, the same north star the CLI report leads with.
// It used to be the full-text-open rate under a key literally named north_star, which made one word
// mean two things across the two entry points. The open rate did not go away -- it is the middle
// bar of the funnel beside it, under a name that says what it counts. A residual that was never
// replayed renders as unavailable rather than as 0%: nothing measured and nothing left are opposite
// readings and the figure cannot tell them apart.
const residual=report.quality.false_delivery;const residualRate=residual&&residual.status==='measured'?residual.residual_rate:null;const residualFigure=pct(residualRate);
...
let hero='<section class="masthead"><div><h2 class="t-md">'+esc(t('north_star'))+'</h2>'+heroFigure(residualFigure)+'<p class="prose">'+esc(residualFigure?t('north_star_help'):t('no_residual'))+'</p></div>'
...
{name:t('fulltext_open'),value:pct(chain(deliveryRate,openRate)),width:share(chain(deliveryRate,openRate))},
...
metric(t('active_memories'),num(health.corpus.active_topics),t('quota_limit')+' '+num(locks.quota.value.max_active_l3)+' '+t('unit_items'))+metric(t('corpus_size'),bytes(health.corpus.active_bytes))+'</div><div class="prose">'+esc(t('zero_net_growth'))+'</div>':empty(null,t('quota_missing'));
  const maintenance=overview.report.maintenance;const maintenanceBody='<div class="stats">'+metric(t('builds'),num(maintenance.builds))+metric(t('failed'),num(maintenance.failed_builds))+'</div>'
// Three of the four tiles here used to come from the maintenance scan, whose producer was removed
// with the unattended governance layer. They rendered as "unavailable" forever, which reads as a
// collector somebody still has to wire in rather than as a mechanism that was deliberately deleted.
+'<div class="prose">'+esc(t('maintenance_help'))+'</div>';
  ...
  // What governance means in this build: a quota that refuses, a review date that comes due, and an
  // index that either rebuilt or did not. The candidate queue, review material, promotion
  // observation window, quarantine ledger and quota settlement cards were removed with the
  // mechanisms behind them -- every one of them had been rendering an empty state since, and one of
  // them told the reader to run 'ownmem candidates scan', a command this package no longer has.
  // NOTE: this whole module body is a template literal, so no backticks in these comments.
  return '<div class="grid">'+card(t('quota'),quotaBody,'','c6')+card(t('maintenance'),maintenanceBody,'','c6')+card(t('review_due'),reviewBody,'','c12')+'</div>'}
