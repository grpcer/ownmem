...
// A queue view is a prompt to go look, not the queue itself; the command prints all of it.
const REVIEW_QUEUE_PREVIEW = 8;
...
function collectFeedbackQueue({ root, memoryDir = '.claude/memory' }) {
  ...
  return {
    ...
    pending: inbox.entries
      .filter(entry => !FEEDBACK_CORRECT_VERDICTS.has(entry.verdict))
      .map(entry => ({
        line: entry.line,
...
export function collectDashboardOverview({ root, memoryDir = '.claude/memory', since = '7d', now = new Date() } = {}) {
  ...
  return {
    ...
    evolution: readMemoryEvolutionState({ root }),
  };
}
...
/**
 * The review queue, as read-only as the rest of this console's governance view.
 *
 * The console's write surface is deliberately narrow -- embedding configuration and the jobs that
 * follow from it -- and declining a candidate is not configuration, it is a judgement that has to
 * be recorded with a reason. So this shows what is waiting and hands over the command, rather than
 * putting a Decline button next to a lead nobody has read yet.
 *
 * A broken ledger is reported rather than thrown: a corrupt review queue must not take the console
 * down, for the same reason it must not reach recall.
 */
function collectReviewQueue(root) {
  try {
    const ledger = readCandidateLedger({ root });
    const candidates = Object.values(ledger.candidates);
    return {
      available: true,
      error: null,
      waiting: candidates.length,
      rejected: Object.keys(ledger.rejected).length,
      items: candidates.slice(0, REVIEW_QUEUE_PREVIEW).map(candidate => ({
      ...
      preview_cap: REVIEW_QUEUE_PREVIEW,
    };
  } catch (error) {
    return { available: false, error: error.message, waiting: 0, rejected: 0, items: [], preview_cap: REVIEW_QUEUE_PREVIEW };
  }
}
...
export function collectDashboardLocks({ root, memoryDir = '.claude/memory', now = new Date() } = {}) {
  ...
  return {
    ...
    candidates: collectReviewQueue(root),
    evolution: readMemoryEvolutionState({ root }),
    // Promotion governance, read-only for the same reason the review queue above is: approving a
    // change, retiring a memory or clearing a quarantine are judgements that carry a reason, and
    // this console's write surface is embedding configuration. It shows what is waiting and hands
    // over the command.
    //
    // All four surfaces have a collector. Three of them read a ledger that is empty on a machine
    // that has never promoted anything, and they say so rather than rendering a zero: an empty
    // ledger is a fact about this machine, and a panel that cannot tell it from an absent collector
    // is a panel nobody can act on.
    governance: collectMemoryGovernance({ root, memoryDir, now }),
  };
}
