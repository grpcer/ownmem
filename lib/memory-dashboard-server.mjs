...
import { collectMemoryReport, createFalseDeliveryReplay } from './features/report.mjs';
...
/**
 * The console's north star is the one figure on the page that cannot be read out of telemetry: it
 * comes from replaying every recorded `wrong` query against the index on disk, about a second of
 * retrieval on this repository's ledger. Affordable once, not on every poll, so the replay is built
 * once per process and each query's envelope is remembered -- a second request still runs the same
 * summarizer over the same replay contract and pays a Map lookup for it. The memo is keyed on the
 * two inputs that can change the answer (the feedback ledger and the compiled snapshot), both read
 * with stat() rather than opened, and it cannot outlive the process.
 *
 * Only the current window gets it. The residual is a property of the whole ledger rather than of a
 * time range, so computing it for the previous window would cost the same second to produce the
 * identical number and then show it as if it were a period comparison.
 */
let replayMemo = null;
function falseDeliveryReplay({ root, memoryDir }) {
  const key = [
    path.join(root, '.local-test/memory-recall-feedback.jsonl'),
    path.join(root, memoryDir, '.ownmem/index/current'),
    path.join(root, '.ownmem/index/current'),
  ].map((target) => {
    try {
      const stats = statSync(target);
      return `${target}:${stats.size}:${stats.mtimeMs}`;
    } catch {
      return `${target}:absent`;
    }
  }).join('|');
  if (replayMemo?.key === key) return replayMemo.replay;
  const replay = createFalseDeliveryReplay({ root, memoryDir });
  if (!replay) {
    replayMemo = { key, replay: null };
    return null;
  }
  const cache = new Map();
  const memoised = (query) => {
    if (cache.has(query)) return cache.get(query);
    const envelope = replay(query);
    cache.set(query, envelope);
    return envelope;
  };
  replayMemo = { key, replay: memoised };
  return memoised;
}
...
export function collectDashboardOverview({ root, memoryDir = '.claude/memory', since = '7d', now = new Date() } = {}) {
  ...
  const report = collectMemoryReport({
    root, memoryDir, since, now, health, replayFalseDelivery: falseDeliveryReplay({ root, memoryDir }),
  });
  return {
