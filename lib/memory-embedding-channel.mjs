...
export async function completeMemoryEmbeddingQueries(started, runtime) {
  ...
  // A memory with no usable vector is excluded from the dense lane below, but the ranking weight is
  // still charged against it: its embedding feature scores 0 while that weight stays in the
  // denominator, so "we never measured this one" is spent as "we measured it and it lost". The
  // memories in that position are the newest ones -- the ones written since the last build.
  //
  // Measured on 2026-08-21 by removing two entries from the artifact and replaying the same query:
  // with a vector the expected memory is rank 1 at weight 4 (0.8138) and with the entry missing it
  // leaves the top five entirely, handing rank 1 to a topic scoring 0.7086. At weight 0 both runs
  // are identical, which is why observe mode keeps the ratio allowance -- it changes no ranking.
  //
  // The A/B evaluator already refuses to score a stale artifact and the Console refuses to enable
  // on one. Only the live path tolerated it, so the configuration that shipped was one no evaluator
  // in this system would agree to measure. Weighted mode now holds the same line and falls back to
  // the deterministic ranking with a named reason until the vectors are rebuilt.
  const unusable = [...freshness.documentIds]
    .filter((documentId) => freshness.staleIds.has(documentId) || !started.artifact.entries[documentId]).length;
  if (unusable > 0 && started.config.rrf_weight > 0) {
    return degradedResult(started, 'artifact-stale', {
      planner: runtime.planner,
      stale: freshness.stale,
      total: freshness.total,
