...
export async function evaluateMemoryEmbeddingAb({
  ...
  // The default used to point inside the installed package at a file no release has ever shipped, so
  // the happy path ended in a raw ENOENT that printed where npm had put ownmem. The cases file is
  // project data: it belongs next to the memory it evaluates, and the caller can point elsewhere.
  const resolvedCasesFile = path.resolve(root, casesFile || memoryRecallCasesFile(memoryDir || '.ownmem'));
  if (!existsSync(resolvedCasesFile)) {
    throw new Error(`memory embedding A/B needs a recall cases file: an ownmem-recall-benchmark/v4 JSON document with golden and negative query arrays. None found at ${path.relative(root, resolvedCasesFile)}; write one there or pass --cases <path>`);
  }
  const benchmark = JSON.parse(readFileSync(resolvedCasesFile, 'utf8'));
  if (benchmark.schema !== 'ownmem-recall-benchmark/v4'
      || !Array.isArray(benchmark.golden)
      || !Array.isArray(benchmark.negative)) {
    throw new Error(`memory embedding A/B requires ownmem-recall-benchmark/v4 cases: ${path.relative(root, resolvedCasesFile)}`);
  }
  const allCases = [...benchmark.golden, ...benchmark.negative];
  // These fields land verbatim in the per-case diff the closed report schema validates, so checking
  // them here turns a JSON-Schema dump about /diffs/0 into a statement about the file the author
  // wrote — named by its own array, so "golden case 2" is findable in that file.
  const describeMalformed = (cases, kind, needsExpected) => {
    const bad = cases.findIndex((testCase) => (
      !testCase?.query
      || !['regression', 'challenge', 'holdout'].includes(testCase.partition)
      || typeof testCase.group !== 'string'
      || !testCase.group
      || (needsExpected && expectedNames(testCase.expected).length === 0)
    ));
    return bad === -1 ? null : `${kind} case ${bad + 1}`;
  };
  const malformed = describeMalformed(benchmark.golden, 'golden', true)
    || describeMalformed(benchmark.negative, 'negative', false);
  if (malformed) {
    throw new Error(`memory embedding A/B ${malformed} in ${path.relative(root, resolvedCasesFile)} needs query, partition (regression|challenge|holdout) and group; golden cases also need expected`);
  }
