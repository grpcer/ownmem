...
function outcome(envelope, expected) {
  // Quoted answers first, then pointers, in the order the caller received them. Ranking metrics ask
  // whether retrieval found the memory; the tier that follows -- quote or pointer -- is a statement
  // about confidence and is measured separately. Reading prose alone here made an A/B comparison
  // report a ranking regression whenever a query merely became less confident, which is a change
  // an embedding lane is entitled to make and would have been scored as damage.
