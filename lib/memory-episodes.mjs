...
export function buildMemoryEpisodes(events, { consumedTraceIds } = {}) {
  ...
  for (const event of ordered) {
    ...
    if (event.event === 'recall.completed') {
      // Only the hook surface has turns at all, so seeing one is what upgrades the episode's host
      // from the default. Which host it was comes from the event's agent; before that field
      // existed the surface itself said `claude-hook`, and for those rows it is all there is.
      if (['hook', 'claude-hook'].includes(event.payload.surface)) {
        episode.host = event.agent || (event.payload.surface === 'claude-hook' ? 'claude' : 'generic');
      }
