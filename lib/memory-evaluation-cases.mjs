...
export function memoryEnvelopeOutcome(envelope) {
  ...
  if (['validity', 'applicability', 'risk'].includes(envelope.abstain.gate)) return 'blocked';
  // Pointers are their own outcome, not a kind of silence. A case that asserts the risk gate let a
  // query through has to be able to say so without also demanding prose, which is a separate claim
  // about confidence that those cases were never evidence for.
  return envelope.delivery?.tier === 'pointers' ? 'pointed' : 'abstained';
}

/**
 * Outcomes in which the gates did not refuse the query.
 *
 * `not_blocked` is what several risk cases have always been evidence about -- their rationales say
 * so in as many words: "blocking the whole result set", "refusing it withholds", "what this case is
 * evidence about is the gate's decision". They asserted `delivered` because a two-state protocol
 * left them nothing else to assert; with a middle tier, demanding prose would make them fail for a
 * confidence judgement they never measured.
 */
const MEMORY_UNBLOCKED_OUTCOMES = Object.freeze(['delivered', 'pointed']);

/**
 * Outcomes in which nothing was quoted as an answer.
 *
 * The mirror image of `not_blocked`, and needed for the same reason: a case can be evidence about
 * one boundary without having an opinion about the other. These cases assert that a long
 * development-period question -- several subjects at once, carried by generic words like loop, call,
 * agent or audit -- must not come back as prose from a memory about something else. Which of the two
 * silent outcomes it gets is not what they are evidence about: a pointer is honest ("here is
 * somewhere to look") and so is an abstention, and pinning one of them would make the case fail
 * whenever a neighbouring memory crosses or leaves the relevance floor for unrelated reasons.
 *
 * Kept separate from `abstained` on purpose. Writing `abstained` here would claim the stronger thing
 * -- that nothing may be named at all -- which is not the contract and would be red today.
 */
const MEMORY_UNQUOTED_OUTCOMES = Object.freeze(['pointed', 'abstained']);

/** Expectations that name a set of acceptable outcomes rather than one. */
const MEMORY_COMPOSITE_OUTCOMES = Object.freeze({
  not_blocked: MEMORY_UNBLOCKED_OUTCOMES,
  not_quoted: MEMORY_UNQUOTED_OUTCOMES,
});
...
export function checkMemoryBehavioralCase(envelope, testCase) {
  ...
  const problems = [];
  // Pointers name memories too, so a case asking which memory came first, or which must never
  // surface, is answered over whatever the envelope actually handed the caller.
  const ids = [
    ...envelope.results.map((result) => result.memory_id),
    ...(envelope.pointers || []).map((pointer) => pointer.memory_id),
  ];
  const outcome = memoryEnvelopeOutcome(envelope);
  const accepted = MEMORY_COMPOSITE_OUTCOMES[expect.outcome];
  const outcomeMatches = accepted ? accepted.includes(outcome) : outcome === expect.outcome;
  if (!outcomeMatches) {
  ...
  if (expect.trust) {
    // Searched across both surfaces: a pointer carries non-nominal trust for the same reason a
    // result does, so a case asserting the label is checked wherever the envelope put the memory.
    const carriers = [...envelope.results, ...(envelope.pointers || [])];
    const target = expect.trust.memory_id
      ? carriers.find((carrier) => carrier.memory_id === expect.trust.memory_id)
      : carriers[0];
    if (!target) problems.push(`no result to check trust on; got ${ids.join('|') || '(abstain)'}`);
      ...
      for (const field of ['lifecycle', 'authority', 'action_risk', 'integrity']) {
        if (expect.trust[field] === undefined) continue;
...
/**
 * Query groups that name a symbol or a path already in the corpus. They prove the indexer can
 * look itself up, not that a person describing a symptom would be answered. Quality metrics skip
 * them; they stay as smoke.
 */
export const MEMORY_EVALUATION_SELF_PROVING_GROUPS = Object.freeze(['identifier', 'anchor']);

/**
 * Sources whose queries are generated from a commit subject rather than written by a person.
 *
 * A file accumulates dozens of unrelated commits, so most subjects of a memory's `code_evidence`
 * path describe some other change. Such a query has no causal link to the topic it is labelled
 * with, and a recall that misses it is not evidence of anything -- the question was wrong, not the
 * answer. These sources must therefore clear the grounding floor below before they may count.
 */
export const MEMORY_EVALUATION_MACHINE_SOURCES = Object.freeze(['git-history', 'code-evidence']);

/**
 * The share of query terms that must appear in the target memory for the pair to be a question
 * about that memory at all.
 *
 * Measured, not picked: on the 2026-09-19 corpus the hand-written `curated` cases sit at p10 0.313
 * and the commit-subject cases at p90 0.313, so the two populations part company almost exactly
 * here. A floor of 0.30 keeps 67 of 73 curated cases and drops roughly nine in ten commit-subject
 * cases. Coverage rather than Jaccard because a long debug lesson dilutes Jaccard toward zero for
 * good and bad queries alike -- that is why the leak gate alone never separated these two.
 */
export const MEMORY_EVALUATION_GROUNDING_MIN = 0.3;

/**
 * How much of the query is actually about this memory.
 *
 * Deliberately asymmetric to the leak gate: leakage asks whether the memory was copied into the
 * question, grounding asks whether the question is about the memory. A query can fail both (a
 * commit subject for an unrelated change) or neither (a person describing the symptom in their own
 * words), so one similarity number cannot stand in for both.
 */
export function memoryEvaluationGrounding(query, { body = '', limit = MEMORY_EVALUATION_GROUNDING_MIN } = {}) {
  const queryTokens = memoryTokenSet(query);
  const bodyTokens = memoryTokenSet(body);
  let shared = 0;
  for (const token of queryTokens) if (bodyTokens.has(token)) shared += 1;
  const coverage = queryTokens.size === 0 ? 0 : shared / queryTokens.size;
  return {
    schema: 'ownmem.evaluation.grounding/v1',
    limit,
    grounded: coverage >= limit,
    coverage: Number(coverage.toFixed(4)),
  };
}

/**
 * Sources that restate something already in the corpus instead of describing a symptom.
 *
 * blind-symptom copies an L2 index line or the opening paragraph. code-evidence is a bare filename
 * from the topic's own `code_evidence`, so its terms are guaranteed to appear in the memory: it
 * scores high on grounding for a reason that has nothing to do with whether a person describing
 * that problem would be answered. Both stay as smoke, exactly like identifier and anchor.
 */
export const MEMORY_EVALUATION_SELF_PROVING_SOURCES = Object.freeze(['blind-symptom', 'code-evidence']);

export function memoryEvaluationIsQualityGolden(testCase) {
  if (MEMORY_EVALUATION_SELF_PROVING_GROUPS.includes(testCase.group)) return false;
  if (MEMORY_EVALUATION_SELF_PROVING_SOURCES.includes(testCase.source)) return false;
  // A machine-generated query counts only once it has been shown to be about its own answer. The
  // harvester records the measurement on the case so this stays auditable in the fixture and does
  // not silently depend on re-reading the corpus at scoring time.
  if (MEMORY_EVALUATION_MACHINE_SOURCES.includes(testCase.source)) return testCase.grounded === true;
  return true;
}

export const MEMORY_EVALUATION_CORPUS_LEAK_LIMIT = MEMORY_EVALUATION_HOLDOUT_SIMILARITY_LIMIT;

export const MEMORY_EVALUATION_EXTERNAL_SOURCES = Object.freeze([
  'git-history',
  'code-evidence',
  'blind-symptom',
  'feedback-miss',
...
/** Holdout may only be filled from these. L2/body copies are not held out. */
export const MEMORY_EVALUATION_HOLDOUT_SOURCES = Object.freeze([
  'git-history',
  'feedback-miss',
  'feedback-wrong',
]);

export const MEMORY_EVALUATION_SPLIT_METHOD = 'external-prefer-topic-groups';
/**
 * 20260919 corrects the 20260918 cut that put L2 index copies in holdout.
 *
 * The seed is not a version number and is deliberately not bumped when the corpus is cleaned. It
 * exists so one corpus always cuts the same way; a different corpus cutting differently under the
 * same seed is the mechanism working, and the reason for that corpus change belongs in the lock's
 * `rationale`, where a reader can weigh it, rather than hidden in a new number here.
 */
export const MEMORY_EVALUATION_SPLIT_SEED = 20260919;
...
/**
 * Query vs the target memory's triggers and prose, not vs other queries.
 *
 * Compared per field so a two-token identifier cannot hide behind a long body: Jaccard against the
 * whole body of a debug lesson is almost always small, which would let "git stash" through. A
 * query that is a rewrite of one trigger is the self-proving case this gate exists to refuse.
 */
export function memoryEvaluationCorpusLeak(query, {
  triggers = [],
  description = '',
  body = '',
  index_lines = [],
  limit = MEMORY_EVALUATION_CORPUS_LEAK_LIMIT,
} = {}) {
  const queryTokens = memoryTokenSet(query);
  const fields = [
    ...triggers.map((text) => ({ field: 'trigger', text })),
    { field: 'description', text: description },
    { field: 'body', text: String(body || '').slice(0, 1200) },
    ...index_lines.map((text) => ({ field: 'index', text })),
  ].filter((item) => String(item.text || '').trim());
  let nearest = { field: null, similarity: 0 };
  for (const item of fields) {
    const similarity = jaccard(queryTokens, memoryTokenSet(item.text));
    if (similarity > nearest.similarity) nearest = { field: item.field, similarity };
  }
  return {
    schema: 'ownmem.evaluation.corpus-leak/v1',
    limit,
    leaked: nearest.similarity >= limit,
    nearest_field: nearest.field,
    similarity: Number(nearest.similarity.toFixed(4)),
  };
}

export function memoryEvaluationRejectLeakingCase(testCase, corpus) {
  const leak = memoryEvaluationCorpusLeak(testCase.query, corpus);
  if (!leak.leaked) return null;
  return {
    case_id: memoryRecallCaseId(testCase, 'golden', 0),
    field: leak.nearest_field,
    similarity: leak.similarity,
    limit: leak.limit,
  };
}

function expectedNameList(testCase) {
  if (testCase.expected === undefined) return [];
...
function findComponent(parent, name) {
  ...
  const root = parent.get(name);
  if (root !== name) parent.set(name, findComponent(parent, root));
  return parent.get(name);
}

/**
 * Cases that share any expected topic stay together. Splitting them across tuning and holdout is
 * how a synonym of a tuning case lands in the holdout and the split stops measuring anything.
 */
export function memoryEvaluationTopicComponents(cases) {
  const parent = new Map();
  for (const item of cases) {
    const names = expectedNameList(item);
    if (names.length === 0) continue;
    for (let index = 1; index < names.length; index += 1) {
      const left = findComponent(parent, names[0]);
      const right = findComponent(parent, names[index]);
...
/**
 * Assign each topic-connected component to tuning or holdout.
 *
 * External-sourced components fill holdout first. Same-day curated components are shuffled with
 * `seed` and fill the remainder. A topic never appears in both partitions. The assignment map is
 * what the quality lock stores so a later run can expand but cannot reshuffle.
 */
export function memoryEvaluationSplitByTopic(cases, {
  seed,
  holdoutRatio = 0.2,
  preferSources = MEMORY_EVALUATION_HOLDOUT_SOURCES,
  assignments = null,
} = {}) {
  if (!Number.isInteger(seed) || seed < 0) throw new Error('split seed must be a non-negative integer');
  const preferred = new Set(preferSources);
  const components = memoryEvaluationTopicComponents(cases);
  const caseKey = new Map();
  for (const component of components) {
    for (const item of component.items) caseKey.set(item, component.key);
  }
  const locked = assignments ? { ...assignments } : {};
  const random = mulberry32(seed);
  const topicNames = (component) => [...new Set(component.items.flatMap(expectedNameList))];
  const componentSide = (component) => {
    const sides = new Set(topicNames(component).map((name) => locked[name]).filter(Boolean));
    if (sides.size > 1) throw new Error(`split lock assigns topics in one component to both sides: ${topicNames(component).join(', ')}`);
    return [...sides][0] || null;
  };
  const unlocked = components.filter((component) => !componentSide(component));
  // A component qualifies for holdout only if it carries a case that can actually carry a metric.
  // Source alone is not enough: most commit-subject cases are about some other change that touched
  // the same file, and a holdout filled with those measures how often the ranker guesses a wrong
  // question right.
  const preferredUnlocked = unlocked.filter((component) => (
    component.items.some((item) => preferred.has(item.source) && memoryEvaluationIsQualityGolden(item))
  ));
  const remainder = unlocked.filter((component) => !preferredUnlocked.includes(component));
  shuffleInPlace(preferredUnlocked, random);
  const targetHoldout = Math.max(1, Math.round(components.length * holdoutRatio));
  let holdoutCount = components.filter((component) => componentSide(component) === 'holdout').length;
  const next = { ...locked };
  const stamp = (component, side) => {
    for (const name of topicNames(component)) next[name] = side;
  };
  for (const component of components) {
    const side = componentSide(component);
    if (side) stamp(component, side);
  }
  // Only grounded git-history / feedback fill holdout. Remainder (curated, L2 copies, filename
  // probes, ungrounded commit subjects) stays in tuning so a later stage cannot shop for a
  // flattering held-out rewrite of the index.
  for (const component of preferredUnlocked) {
    if (holdoutCount < targetHoldout) {
      stamp(component, 'holdout');
      holdoutCount += 1;
    } else {
      stamp(component, 'tuning');
    }
  }
  for (const component of remainder) stamp(component, 'tuning');
  const partitioned = cases.map((item) => {
    const names = expectedNameList(item);
    const side = names.length > 0 ? next[names[0]] : next[caseKey.get(item)];
    const assignment = side || 'tuning';
    const partition = assignment === 'holdout' ? 'holdout' : (item.partition === 'challenge' ? 'challenge' : 'regression');
    return { ...item, partition };
  });
  return {
    schema: 'ownmem.evaluation.split/v1',
    method: MEMORY_EVALUATION_SPLIT_METHOD,
    seed,
...
/**
 * A quality lock that already recorded a split may grow, but may not move a topic from one side
 * to the other. Re-cutting is how a later stage shops for a flattering holdout.
 */
export function memoryEvaluationSplitLockViolation(lock, nextAssignments) {
  const previous = lock?.split?.assignments || null;
  if (!previous) return null;
  if (lock.split.method !== MEMORY_EVALUATION_SPLIT_METHOD) {
    return `split method changed from ${lock.split.method} to ${MEMORY_EVALUATION_SPLIT_METHOD}`;
  }
  if (lock.split.seed !== nextAssignments.seed) {
    return `split seed changed from ${lock.split.seed} to ${nextAssignments.seed}`;
  }
  const moved = [];
  for (const [topic, side] of Object.entries(previous)) {
    if (nextAssignments.assignments[topic] && nextAssignments.assignments[topic] !== side) {
      moved.push(`${topic}:${side}->${nextAssignments.assignments[topic]}`);
    }
  }
  return moved.length > 0 ? `split reassigned topics: ${moved.join(', ')}` : null;
}

export function memoryEvaluationGateScoreSeparation({ good, bad }) {
  const percentile = (values, ratio) => {
    if (values.length === 0) return null;
    const sorted = [...values].sort((left, right) => left - right);
    return sorted[Math.max(0, Math.ceil(sorted.length * ratio) - 1)];
  };
  const goodMetric = memoryEvaluationMetric({
    numerator: good.length,
    denominator: good.length,
    sample: good.length,
  });
  const badMetric = memoryEvaluationMetric({
    numerator: bad.length,
    denominator: bad.length,
    sample: bad.length,
  });
  const goodP10 = percentile(good, 0.1);
  const goodP50 = percentile(good, 0.5);
  const badP50 = percentile(bad, 0.5);
  const badP90 = percentile(bad, 0.9);
  const measurable = goodMetric.status === MEMORY_EVALUATION_MEASURED
    && badMetric.status === MEMORY_EVALUATION_MEASURED
    && goodP10 !== null
    && badP90 !== null;
  return {
    schema: 'ownmem.evaluation.gate-score-separation/v1',
    status: measurable ? MEMORY_EVALUATION_MEASURED : MEMORY_EVALUATION_INSUFFICIENT_EVIDENCE,
    good: { n: good.length, p10: goodP10, p50: goodP50 },
    bad: { n: bad.length, p50: badP50, p90: badP90 },
    // Positive when the weakest good delivery still outscores the strongest bad one. Overlap is
    // the structural ceiling the ranker's relative gate cannot see; it is reported, not dressed
    // as a threshold the system already meets.
    overlap: measurable ? Number((badP90 - goodP10).toFixed(4)) : null,
  };
}
