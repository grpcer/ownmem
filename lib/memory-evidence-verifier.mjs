...
function verifyCodeEvidence(root, evidence, { requireSymbol = false } = {}) {
  ...
  // The slice matched something that is not a definition -- a call site, or a comment that names
  // the symbol. The fingerprint then tracks the wrong block: it neither rots nor errors, it just
  // silently vouches for a different piece of code, which is worse than no anchor because the
  // receipt still reads as verified. `docs-freshness` has warned about this shape since 2026-07-29;
  // the memory receipts were reading the same flag off the same function and throwing it away.
  //
  // Advisory rather than blocking: the file and a mention of the symbol are both still there, so
  // this is "your anchor is pointed at the wrong thing", not "the thing is gone". Measured
  // 2026-09-18 over 178 distinct anchors, 3 of them were weak.
  if (anchor.weakSymbol) return advisory('symbol-not-a-definition');
