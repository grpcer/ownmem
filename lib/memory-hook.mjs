 ...
 * actually in use, and each exits on its own five-minute idle timer.
 *
...
// of both.
const HOOK_UNVERIFIED_NOTICE = 'At least one memory below is not fully verified (see result.trust: advisory authority, non-passed integrity, or a lifecycle other than active).';
...
export function formatClaudePreToolHook(response) {
  ...
  const advice = memoryTrustAdviceLines(response.result.results.map(result => result.trust));
