...
import { validateMemoryIndexManifest } from './memory-contracts.mjs';
...
export const MEMORY_ARTIFACT_SPECS = Object.freeze({
  ...
  ranking: Object.freeze({ file: 'ranking.json', schema: 'ownmem-index.ranking/v4' }),
});
...
export function validateSnapshotDirectory(directory) {
  ...
  for (const [kind, spec] of Object.entries(MEMORY_ARTIFACT_SPECS)) {
    ...
    // Parsed from the bytes already in hand. Re-opening the file to parse it read the whole 12 MB
    // snapshot a second time on a path a cold recall runs once per process.
    let artifact;
    try {
      artifact = JSON.parse(bytes.toString('utf8'));
    } catch (error) {
      throw new Error(`memory index ${kind} artifact is unreadable: ${error.message}`);
    }
    // The JSON Schema check that used to run here is not repeated. The compiler validates every
    // artifact against the same schema before it writes it (memory-compiler.mjs, guarded by
    // memory-compile-self-test.mjs), and the two lines above have just proved these bytes are
    // exactly the bytes it validated -- same length, same SHA-256 as the manifest declares, with
    // the manifest itself checked against its own schema and its declared id. Re-deriving a verdict
    // that the checksum already implies cost 86 ms of Ajv compilation and validation per cold
    // recall, 20% of it. Reader/writer contract drift is a different question and still answered
    // above: `declared.schema !== spec.schema` fails the read before any of this.
...
function replaceSymlink(indexRoot, pointer, target) {
  ...
  try {
    // Windows does not replace an existing symlink with renameSync. Publication ordering keeps the
    // other pointer readable while this one is briefly absent, so remove only the link itself and
    // restore it if the second rename fails. POSIX keeps the single atomic rename path.
    if (process.platform === 'win32' && existsSync(pointerPath)) {
      const previousTarget = readlinkSync(pointerPath);
      unlinkSync(pointerPath);
      try {
        renameSync(temporary, pointerPath);
      } catch (error) {
        try {
          symlinkSync(previousTarget, pointerPath);
        } catch (restoreError) {
          throw new AggregateError([error, restoreError], `failed to replace and restore ${pointer} pointer`);
        }
        throw error;
      }
      return;
