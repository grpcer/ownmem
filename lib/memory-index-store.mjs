...
export const MEMORY_ARTIFACT_SPECS = Object.freeze({
  documents: Object.freeze({ file: 'documents.json', schema: 'ownmem-index.documents/v1' }),
  // readable_schemas lists older encodings this reader still accepts. They are upgraded in memory by
  // upgradeArtifact below, so everything downstream only ever sees the current shape.
  postings: Object.freeze({
    file: 'postings.json',
    schema: 'ownmem-index.postings/v2',
    readable_schemas: Object.freeze(['ownmem-index.postings/v1']),
  }),
...
// v1 stored a full topic name in every posting and a full-precision float in every term. v2 interns
// both. Upgrading on read keeps the previous-snapshot fallback working across the version bump: the
// pointer may still reference a v1 directory long after the current one has been recompiled.
function upgradePostingsV1(artifact) {
  const documentIds = [...new Set(artifact.terms.flatMap((term) => term.postings.map(([id]) => id)))]
    .sort((left, right) => left.localeCompare(right, 'en'));
  const documentIndex = new Map(documentIds.map((id, index) => [id, index]));
  const idfTable = [];
  const idfIndex = new Map();
  const terms = artifact.terms.map((term) => {
    let idf = idfIndex.get(term.idf);
    if (idf === undefined) {
      idf = idfTable.length;
      idfTable.push(term.idf);
      idfIndex.set(term.idf, idf);
    }
    return {
      term: term.term,
      idf,
      postings: term.postings.map(([id, fields]) => [documentIndex.get(id), fields]),
    };
  });
  return {
    ...artifact,
...
function upgradeArtifact(kind, declaredSchema, artifact) {
  if (kind === 'postings' && declaredSchema === 'ownmem-index.postings/v1') return upgradePostingsV1(artifact);
  return artifact;
}
...
export function validateSnapshotDirectory(directory) {
  ...
  for (const [kind, spec] of Object.entries(MEMORY_ARTIFACT_SPECS)) {
    ...
    const readable = declared.schema === spec.schema
      || (spec.readable_schemas || []).includes(declared.schema);
    if (declared.file !== spec.file || !readable) {
      throw new Error(`memory index ${kind} artifact contract does not match the current reader`);
    }
    ...
    // Validated against the schema it was written with, then upgraded. Checking a v1 file against
    // the v2 schema would reject a snapshot that is perfectly intact.
    const legacySchemaId = declared.schema === spec.schema ? null : declared.schema;
    validateMemoryIndexArtifact(kind, artifact, { schemaId: legacySchemaId });
    if (artifactRecordCount(kind, artifact) !== declared.records) {
      throw new Error(`memory index ${kind} artifact record count mismatch`);
    }
    artifacts[kind] = upgradeArtifact(kind, declared.schema, artifact);
