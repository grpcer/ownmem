...
- Recall answers in one of three ways. It quotes a memory when it is confident; it lists the closest
  memories without quoting them when it is not; and it abstains when there is nothing to name. A
  list without quotes is not a weaker answer to the same question -- it is the runtime declining to
  vouch, so open the files it names and decide for yourself rather than repeating the summaries.
- An abstention means "nothing recorded", not "look harder".
...
export function detectMemoryHosts(root) {
  const hosts = [];
  // A root CLAUDE.md or GEMINI.md is as much a host marker as the settings directory, and the more
  // common one: a repository with only CLAUDE.md used to install for every host except Claude Code.
  if (existsSync(path.join(root, '.claude')) || existsSync(path.join(root, 'CLAUDE.md'))) hosts.push('claude');
  if (existsSync(path.join(root, 'AGENTS.md'))) hosts.push('codex');
  if (existsSync(path.join(root, '.gemini')) || existsSync(path.join(root, 'GEMINI.md'))) hosts.push('gemini');
 ...
 *
 * Whatever this table holds is what "installed" means; anything missing from it is a surface a
 * consumer silently does not have. The drift is invisible from the outside, so the table is the
 * only place the answer lives.
 *
 * It is deliberately short. Three collection hooks were removed in 2.0 -- PostToolUse and
 * PostToolUseFailure on `Bash|WebFetch|WebSearch|mcp__.*`, and `hook stop` -- because the events
 * they wrote (`command.completed`, `external.context.observed`, `correction.observed`) had no
 * reader anywhere: no report, no metric, no gate. They cost a Node process per shell command and
 * per turn to fill a file nothing opened. Adding a hook here means naming what reads it.
 */
const MEMORY_HOOK_SPECS = Object.freeze([
  ...
  // Start the resident daemon while the session is opening, so the first Edit or Write does not pay
  // for it. Measured 2026-09-18: an Edit hook costs 530ms against a cold daemon, 110ms against a
  // warm one.
  { event: 'SessionStart', matcher: null, argument: 'hook warm', timeout: 5 },
...
/**
 * Hooks this installation used to register and no longer does.
 *
 * Removal has to be explicit, because installing is additive by design -- everything not owned is
 * copied through untouched, and an entry that simply stopped appearing in MEMORY_HOOK_SPECS would
 * be left in place forever. That is not a cosmetic leftover: `ownmem hook posttool` now exits
 * non-zero with "unknown memory hook command", so an upgraded v1 installation would fire a failing
 * hook on every single Bash call until someone edited the file by hand.
 *
 * Matched exactly like an active spec -- same event, same matcher, ownership decided by the
 * argument the command opens with -- so a hand-written hook on the same event is never touched.
 */
export const RETIRED_MEMORY_HOOK_SPECS = Object.freeze([
  { event: 'PostToolUse', matcher: 'Bash|WebFetch|WebSearch|mcp__.*', argument: 'hook posttool', legacyArgument: 'posttool' },
  { event: 'PostToolUseFailure', matcher: 'Bash|WebFetch|WebSearch|mcp__.*', argument: 'hook posttool', legacyArgument: 'posttool' },
  { event: 'Stop', matcher: null, argument: 'hook stop', legacyArgument: 'stop' },
]);

function removeRetiredHooks(settings, command) {
  let removed = 0;
  for (const spec of RETIRED_MEMORY_HOOK_SPECS) {
    let found = findOwnedHook(settings, spec, command);
    while (found) {
      found.entry.hooks.splice(found.index, 1);
      removed += 1;
      found = findOwnedHook(settings, spec, command);
    }
  }
  // An entry left with no hooks is dead weight the host still walks; drop it, and drop an event
  // key that has no entries left.
...
function ensureHostHooks({ root, command, host, relative, check }) {
  ...
  if (check) {
    const present = MEMORY_HOOK_SPECS.every(spec => {
      ...
      // A retired hook still present is drift too. Reporting `valid` while a hook that exits
      // non-zero fires on every Bash call is exactly the invisible state this check exists to catch.
      && RETIRED_MEMORY_HOOK_SPECS.every(spec => findOwnedHook(settings, spec, command) === null);
    return { path: relative, status: present ? 'valid' : 'drifted', sha256: present ? fingerprint : null };
  }
  removeRetiredHooks(settings, command);
...
function telemetryDisclosure({ command, memoryDir }) {
  return [
    `the daily pass writes counted telemetry packages under the local observability tree and does not commit them`,
  ];
}

/**
 * `core.hooksPath` left behind by a version that managed it, now pointing nowhere.
 *
 * Reported rather than repaired, and reported as a note rather than as drift. Repairing it would mean
 * writing to a git setting this version deliberately does not own -- the reader may have pointed it
 * somewhere of their own since -- and calling it drift would fail `init --check` over a file this
 * tool no longer manages. What it must not do is stay quiet: a hooksPath that does not resolve
 * disables every repository hook without a word from git.
 */
function retiredHooksPathNote({ root, memoryDir, command }) {
  let configured = '';
  try {
    ...
    // Not a git checkout, git missing, or the setting is unset: nothing to say.
    return null;
  }
  if (!configured) return null;
  const resolved = path.resolve(root, configured);
  const retired = path.resolve(root, memoryDir, 'git-hooks');
  const exists = existsSync(resolved) && statSync(resolved).isDirectory();
  if (resolved !== retired && exists) return null;
  if (resolved !== retired) {
    return `core.hooksPath is set to ${configured}, which is not a directory, so git is installing no`
      + ` repository hooks at all -- including any of your own. Point it somewhere real or clear it:`
      + ' `git config --unset core.hooksPath`';
  }
  return `core.hooksPath still points at ${configured}, which an earlier version of ${command} set and this`
    + ` version no longer manages${exists ? '' : ' (and that directory no longer exists, so git is installing no repository hooks at all -- including any of your own)'}.`
    + ' Clear it with `git config --unset core.hooksPath`';
}
...
export function initializeMemoryRepository({
  ...
  // Said on every run, check or update, because it is the one thing this version cannot repair and
  // the one that fails silently. 0.6.0 pointed `core.hooksPath` at <memory-dir>/git-hooks while
  // generating a post-commit script there; this version generates neither and, per the release notes,
  // does not hand the setting back. Measured 2026-09-20 on a real 0.6.0 checkout upgraded in place:
  // once that directory is gone -- a cleanup, a fresh clone, a colleague who read the changelog --
  // git installs no hooks at all from it, so the repository's own pre-commit stopped running and the
  // commit still exited 0. Nothing reported it: `init --check` was green and said nothing. This
  // repository lost hooks that way on 2026-09-18, which is how the failure mode is known at all.
  const orphanedHooksPath = retiredHooksPathNote({
    root: absoluteRoot, memoryDir: selectedMemoryDir, command: selectedCommand,
  });
  if (orphanedHooksPath) notes.push(orphanedHooksPath);
