...
import { CONFIG_DIRECTORIES, resolveMemoryDir } from './memory-paths.mjs';
...
const LEGACY_GENERATED_ID = 'memory-agent-instructions-v1';
const START = id => `<!-- ownmem-generated:start ${id} -->`;
const END = id => `<!-- ownmem-generated:end ${id} -->`;
const LEGACY_START = id => `<!-- memory-generated:start ${id} -->`;
const LEGACY_END = id => `<!-- memory-generated:end ${id} -->`;
...
function locateBlock(content, id, legacyId = id) {
  return locateBoundary(content, START(id), END(id), id)
    || locateBoundary(content, LEGACY_START(legacyId), LEGACY_END(legacyId), legacyId);
}

export function upsertGeneratedBlock(content, id, body, legacyId = id) {
  const block = generatedBlock(id, body);
  const located = locateBlock(content, id, legacyId);
...
function adapterSpecs({ root, command, memoryDir, locale, hosts }) {
  ...
  if (hosts.includes('claude')) {
    ...
    specs.push({ host: 'claude', kind: 'whole', path: '.claude/commands/ownmem.md', legacyPath: '.claude/commands/memory.md', id: 'ownmem-slash-v1', legacyId: 'memory-slash-v1', body: slashBody(command) });
  }
  ...
  if (hosts.includes('codex') || hosts.includes('cursor')) {
    specs.push({
      ...
      legacyId: 'ownmem-codex-skill-v1',
  ...
  if (hosts.includes('gemini')) {
    ...
    specs.push({ host: 'gemini', kind: 'whole', path: '.gemini/commands/ownmem.toml', legacyPath: '.gemini/commands/memory.toml', id: 'ownmem-slash-v1', legacyId: 'memory-slash-v1', body: geminiCommand(command) });
  }
  if (hosts.includes('cursor')) specs.push({
    host: 'cursor',
    kind: 'whole',
    path: '.cursor/rules/ownmem.mdc',
    legacyPath: '.cursor/rules/memory.mdc',
    id: GENERATED_ID,
...
function ensureClaudeHook({ root, command, check }) {
  ...
  const hookCommand = `${command} hook`;
  const legacyHookCommands = new Set(['npx @oriveo/memory hook', 'npx memory hook', 'memory hook']);
  for (const entry of settings.hooks.PreToolUse) {
    if (entry.matcher !== 'Edit|Write' || !Array.isArray(entry.hooks)) continue;
    for (const configuredHook of entry.hooks) {
      if (configuredHook.type === 'command' && legacyHookCommands.has(configuredHook.command)) {
        configuredHook.command = hookCommand;
      }
    }
  }
  const present = settings.hooks.PreToolUse.some(entry => entry.matcher === 'Edit|Write'
    && Array.isArray(entry.hooks)
    && entry.hooks.some(hook => hook.type === 'command' && hook.command === hookCommand));
  if (check) return { path: relative, status: present ? 'valid' : 'drifted', sha256: present ? sha256(hookCommand) : null };
  if (!present) settings.hooks.PreToolUse.push({ matcher: 'Edit|Write', hooks: [{ type: 'command', command: hookCommand, timeout: 5 }] });
  const content = `${JSON.stringify(settings, null, 2)}\n`;
  return { path: relative, status: writeIfChanged(file, content), sha256: sha256(hookCommand) };
}
...
export function initializeMemoryRepository({
  ...
  // The config probe only ever looks in these directories. Installing anywhere else writes a
  // config.json no bare command can find again, so the mismatch has to fail here, loudly.
  if (!CONFIG_DIRECTORIES.includes(selectedMemoryDir)) {
    throw new Error(`memory init supports ${CONFIG_DIRECTORIES.join(' or ')} as the memory directory; '${selectedMemoryDir}' would be invisible to bare commands`);
  }
  ...
  const legacyCommands = new Set(['npx @oriveo/memory', 'npx memory', 'memory']);
  const existingCommand = legacyCommands.has(existingConfig?.command) ? 'npx ownmem' : existingConfig?.command;
  const selectedCommand = command || existingCommand || 'npx ownmem';
  ...
  for (const adapter of adapters) {
    ...
    const legacyId = adapter.legacyId || (adapter.id === GENERATED_ID ? LEGACY_GENERATED_ID : adapter.id);
    ...
    const expected = wholeFile ? `${adapter.body.trim()}\n` : upsertGeneratedBlock(current, adapter.id, adapter.body, legacyId);
    const located = wholeFile ? null : locateBlock(current, adapter.id, legacyId);
    ...
    if (check) {
      ...
      if (migrateLegacyFile) {
        const legacyLocated = locateBlock(current, adapter.id, legacyId);
        // A marker-less whole file is only removable when it sits inside the memory directory:
        // that directory is generated end to end, so ownership is certain without a marker.
        const ownedWholeLegacy = wholeFile
          && posix(path.relative(absoluteRoot, legacyAbsolutePath)).startsWith(`${posix(selectedMemoryDir)}/`);
        if (ownedWholeLegacy || (legacyLocated && current.trim() === current.slice(legacyLocated.start, legacyLocated.end).trim())) {
