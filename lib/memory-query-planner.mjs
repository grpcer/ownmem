...
/**
 * `terms` and `indexedTerms` are kept apart, and the distinction is load-bearing rather than tidy.
 *
 * `terms` is what the QUERY said: tokens the asker typed (or, for an exact hit, the substring of the
 * query that matched an anchor). `indexedTerms` is what the CORPUS said: the indexed token a lane
 * reached the candidate through -- an n-gram neighbour, a fuzzy neighbour, or the name of the seed
 * memory a graph relation came from.
 *
 * They used to share one set, and that had two consequences. The published `matched_terms` field --
 * which callers and humans read as "which of my words matched" -- carried tokens nobody had typed:
 * measured 2026-09-20 on a competitor-survey query naming several memory products, the top candidate
 * reported `android_chat_finalize_race_flash` as a matched term, which is another memory's name
 * arriving from the graph lane. And coverageFloorAllowsCandidate walks this
 * same set looking for a substantial non-compact query word as grounds to waive the coverage floor;
 * a snake_case memory name is always long and always non-compact, so the waiver could fire on a
 * query that contained no qualifying word at all.
 *
 * queryCoverageFeature still gets both, deliberately: an inflected German query word and its indexed
 * form are different postings, and counting only exact query tokens there reports morphology as
 * irrelevance. That is why the fix is two sets rather than dropping the indexed side.
 */
function addCandidate(candidates, documentId, { score, fields = [], terms = [], indexedTerms = [], evidence = [] }) {
  let candidate = candidates.get(documentId);
  if (!candidate) {
    candidate = { documentId, score: 0, fields: new Set(), terms: new Set(), indexed: new Set(), evidence: new Map() };
  ...
  for (const term of indexedTerms) candidate.indexed.add(term);
...
function finalizeCandidates(channel, candidates, limit) {
  return [...candidates.values()]
    ...
    .map((candidate, index) => ({
      ...
      indexed_terms: [...(candidate.indexed || [])].sort(compareText),
...
function fuseQueryVariants(channel, candidateLists, k, limit) {
  ...
  for (const candidates of candidateLists) {
    for (const candidate of candidates) {
      ...
      for (const term of candidate.indexed_terms || []) aggregate.indexed.add(term);
...
function graphRelations(planner, seedDocumentId) {
  ...
  for (const edge of planner.edgesByNode.get(seedNode) || []) {
    ...
    // An L2 file is a routing index, not a semantic relation. Treating it as a relay turns one
    // exact symbol hit into every memory in the same platform bucket (currently up to the channel
    // cap of 50), drowning the useful wiki/code neighbors and making ranking scale with a category.
    if (neighbor.startsWith('l2:')) continue;
  ...
  for (const seed of eligibleSeeds) {
    for (const relation of graphRelations(planner, seed.document_id)) {
      ...
      addCandidate(candidates, relation.target, {
        ...
        // The seed memory's own name, which is corpus-side by construction: a graph relation is
        // reached from another memory, never from something the asker typed.
        indexedTerms: [seed.document_id],
...
export function planMemoryQuery(planner, query, { limit, embeddingResponse } = {}) {
  ...
  // Exact path/basename suffixes already return every topic anchored to that file. Expanding one
  // of those topics through the graph is both redundant and dangerously broad: a shared L2 route
  // is a graph node, so one exact file hit can fan out to fifty unrelated topics in the same area.
  // That made use-time trust validation dominate every matching edit. Mixed queries retain all
  // lanes because their prose may genuinely need lexical and graph expansion.
  const strictPath = classifications.length === 1 && classifications[0] === 'path';
  // A single-class identifier/error with exact structural evidence has already resolved the thing
  // the user named. Broad lexical and typo-recovery lanes can only add noise; a genuine typo has no
  // exact structural hit and therefore keeps every recovery lane. Mixed queries keep them because
  // the surrounding prose may be asking about a different, related memory.
  const exactStructuralLookup = classifications.length === 1
    && ['identifier', 'error'].includes(classifications[0])
    && exact.some((candidate) => candidate.evidence.some((item) => GRAPH_SEED_KINDS.has(item.kind)));
  const exactOnly = strictPath || exactStructuralLookup;
  const bm25f = exactOnly ? [] : bm25fCandidates(planner, queryTokens, candidateLimit);
  const ngram = exactOnly ? [] : ngramCandidates(planner, raw, queryTokens, candidateLimit);
  const fuzzy = exactOnly ? [] : fuzzyCandidates(planner, raw, classifications, candidateLimit);
  const graph = strictPath ? [] : graphCandidates(planner, exact, candidateLimit);
