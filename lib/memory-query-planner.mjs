...
export function createMemoryQueryPlanner(snapshot, { root = null } = {}) {
...
function asciiQueryFragments(query) {
  const tokens = tokenizeMemoryText(query).filter((token) => /^[a-z0-9]{4,64}$/.test(token));
...
function ngramCandidates(planner, query, queryTokens, limit) {
  ...
  for (const queryTerm of asciiQueryFragments(query)) {
...
export function planMemoryQuery(planner, query, { limit, embeddingResponse } = {}) {
  ...
  const queryTokens = [...new Set(tokenizeMemoryText(raw))];
