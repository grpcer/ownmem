...
function aggregateCandidates(planner, planned) {
  ...
  for (const channel of CHANNEL_ORDER) {
    ...
    for (const candidate of planned.channels[channel]) {
      ...
      // Kept apart all the way through: see addCandidate in the planner for why one set was wrong.
      for (const term of candidate.indexed_terms || []) aggregate.indexedTerms.add(term);
...
function lexicalFeatures(candidate, query, queryContext) {
  ...
  return {
    ...
    // Both sides on purpose, and this is the one place that wants both: a query word and the
    // indexed form it inflects to are different postings, so counting only exact query tokens here
    // reports morphology as irrelevance (measured 2026-08-27: German and Polish failed outright).
    // Everything else -- the published matched_terms, the coverage-floor waiver -- reads the
    // query side alone.
    query_span_coverage: queryCoverageFeature(
      queryContext.coverage, matched, [...candidate.terms, ...candidate.indexedTerms],
    ),
...
const SUBSTANTIAL_NONCOMPACT_TERM = 4;
const EVIDENCED_COVERAGE_EXEMPTION = 0.75;
const EVIDENCED_COVERAGE_MIN_SPAN = 0.3;

/**
 * Whether a scored candidate may pass the coverage floor.
 *
 * Zero means the gate is unarmed. Identifier/path/error lookups are exempt because a single-token
 * query has coverage 0 or 1 and the exact lane often misses a camelCase symbol that BM25 still
 * ranked correctly. A non-compact word of four or more characters is the same kind of evidence.
 * Strongly evidenced memories (code-backed) with at least some surface overlap are the
 * colloquial-rewrite case: the query shares little form with the answer, and the floor was
 * never about them. Evidence alone is not an exemption -- a well-backed memory that only
 * shares a two-character fragment is still a false positive. Natural-language queries that
 * only share compact fragments with a weakly evidenced memory are exactly the in-domain
 * false positives the floor exists to stop.
 */
export function coverageFloorAllowsCandidate({
  coverageFloor,
  classifications = [],
  features,
  terms = [],
  ...
  if (features.exact_anchor > 0) return true;
  if (!classifications.includes('natural')) return true;
  for (const term of terms) {
    if (typeof term === 'string'
        && term.length >= SUBSTANTIAL_NONCOMPACT_TERM
        && !memoryScriptIsCompact(term)) {
      return true;
    }
  }
  if (features.evidence >= EVIDENCED_COVERAGE_EXEMPTION
      && features.query_span_coverage >= EVIDENCED_COVERAGE_MIN_SPAN) {
    return true;
  }
  return features.query_span_coverage >= coverageFloor;
}
...
export function rankPlannedMemoryQuery(planner, query, planned, { limit, featureQueries, trustEvaluator = null, now = new Date() } = {}) {
  ...
  // anchor cases from being dropped by a rule that was never about them. Identifier classification
  // and substantial non-compact tokens extend that bypass to the camelCase symbols BM25 ranks
  // without an exact posting; see coverageFloorAllowsCandidate.
  const coverageFloor = planner.ranking.coverage_floor;
  const scored = fused.filter((candidate) => candidate.gateScore >= threshold);
  //
  // Both term sets are passed here, and that is a deliberate hold rather than an oversight. The
  // substantial-word waiver is documented as "a non-compact word of four or more characters is the
  // same kind of evidence [as an exact anchor]", which means a word the ASKER used; feeding it the
  // corpus side lets a memory waive the floor on its own plural. Measured 2026-09-20 on one recorded
  // query mixing a keychain phrase in a compact script with the token LIVE_KEY: the only query-side
  // matches on pentest_red_lines are `key` (three characters, under the bar) and one compact-script
  // word, and the waiver fires on the indexed `keys`, which nobody typed. So the input is wrong and
  // it is also load-bearing -- restricting it to the query side
  // makes that candidate disappear entirely (below-coverage at span 0.214), and it is a memory a
  // human filed a `correct` receipt for. Over the whole ledger the correction costs exactly that one
  // answer of 79 and buys nothing measurable: every evaluation metric, and the known false-delivery
  // residual at 8/106, is identical either way. Trading a real answer for no measured gain is a
  // relevance-gate decision, so the wrong-but-harmless input stays until there is evidence that
  // settles it. What was fixed is the half with no cost: the published `matched_terms` no longer
  // reports corpus-side tokens (see addCandidate in the planner).
  const relevant = scored.filter((candidate) => coverageFloorAllowsCandidate({
    coverageFloor,
    classifications: planned.classifications,
    features: candidate.features,
    terms: [...candidate.terms, ...candidate.indexedTerms],
  }));

  // Trust is resolved here rather than inside the scoring loop above, and the placement is a
  // measured cost decision, not a stylistic one. Resolving a receipt reads every piece of evidence
  // it declares off disk and re-fingerprints the anchored symbol; measured 2026-09-18 on the hot
  // path, that work was 40% of a recall (fingerprinting alone 21%) while the relevance gates
  // discard most candidates before anything reads the verdict.
  //
  // Nothing above this line may read `candidate.trust`, `candidate.verdicts` or
  // `candidate.blockingGate`: the two relevance gates are query-vs-corpus tests and must stay that
  // way, or a memory would be scored on who signed it. Everything below reads them only through
  // `relevant`, which is why restricting the evaluation to `relevant` is output-identical -- both
  // `trusted`/`trustBlocked` and the blocking-gate verdicts are derived from `relevant` alone.
