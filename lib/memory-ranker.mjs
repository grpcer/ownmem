...
export function rankPlannedMemoryQuery(planner, query, planned, { limit, featureQueries, trustEvaluator = null, now = new Date() } = {}) {
  ...
  const relevant = fused.filter((candidate) => candidate.gateScore >= threshold);
  ...
  return {
    ...
    abstain: {
      ...
      reason: candidates.length > 0
        ? null
        : blockedGate ? `blocked-${blockedGate}` : fused.length === 0 ? 'no-trusted-candidate' : 'below-threshold',
