...
export function rememberMemoryRecall({
  ...
  try {
    ...
    const entry = {
      ...
      topics,
      source: ['hook', 'claude-hook'].includes(source) ? source : 'cli',
    };
...
export class MemoryRecallLedger {
  ...
  /**
   * Resolve which recall delivered a topic. A same-session delivery is the strongest pairing; any
   * other fresh delivery of the same topic counts as recent. Returns null when nothing inside the
   * TTL delivered it -- callers must then refuse to record rather than invent an anchor, because a
   * receipt whose trace does not name the recall it is about is worse than no receipt at all.
   */
  pair({ sessionId = null, memoryId, now = new Date() } = {}) {
    ...
    return {
      ...
      // The producing surface travels with the pairing. Reading it back beats assuming the hook
      // produced every open, which stopped being true once the CLI wrote to the same ledger.
      source: ['hook', 'claude-hook', 'cli'].includes(matched.source) ? matched.source : 'hook',
      match: sameSession ? 'session' : 'recent',
    };
  }

  /**
   * Pair a full-text open with the recall that returned the topic. Opening is not application: this
   * only proves the body was read, which is why `via` is the reporting surface and nothing here
   * upgrades into the outcome receipt stream.
   */
  consume({ sessionId = null, memoryId, now = new Date() } = {}) {
    const paired = this.pair({ sessionId, memoryId, now });
    if (!paired) return null;
    return {
      traceId: paired.traceId,
      snapshotId: paired.snapshotId,
      topics: [memoryId],
      via: paired.source,
      match: paired.match,
    };
