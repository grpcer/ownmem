...
/**
 * Every reason the envelope can carry, kept in step with the protocol schema.
 *
 * Until 2026-09-19 this set was missing the three blocked-* values and the mapper below sent
 * anything unlisted to `no-trusted-candidate`. A trust gate refusing a candidate was therefore
 * filed as "nothing matched" -- the two call for opposite responses, one to write the missing
 * memory and one to look at the gate -- and the production report showed 718 of the former with
 * none of the latter. memory-observability-self-test now walks the protocol enum so a reason added
 * on one side cannot go missing on the other.
 */
const ABSTAIN_REASONS = new Set([
  'no-trusted-candidate', 'below-threshold', 'below-coverage', 'below-content-threshold',
  'blocked-validity', 'blocked-applicability', 'blocked-risk', 'budget-empty', 'all-candidates-excluded',
]);

function normalizedAbstainReason(envelope) {
  if (!envelope.abstain?.abstained) return null;
  const reason = envelope.abstain.reason;
  // An unrecognised reason is recorded as unknown, never as a specific one. Substituting a
  // plausible value is how the last gap stayed invisible for as long as it did: the field was
  // never empty, so nothing looked wrong.
  return ABSTAIN_REASONS.has(reason) ? reason : null;
}
...
export function createMemoryRuntimeObserver(options = {}) {
  ...
  // `claude-hook` is still accepted so an older caller keeps working; new callers say `hook`
  // and let the orthogonal `agent` field name the host.
  const surface = ['hook', 'claude-hook'].includes(options.surface) ? options.surface : 'cli';
  ...
  return {
    recordRecall({ runtime, query, envelope, metrics, execution, episodeId = null } = {}) {
      ...
      // Quoted answers and pointers together: the field records which memories reached the caller,
      // and after the delivery tiers landed a pointer is one of the ways they do. Counting prose
      // alone made a three-pointer answer indistinguishable from silence in the ledger, and the
      // receipt binding test caught it as a mismatch between what the CLI wrote and what the
      // recall event claimed had come back.
      const returnedTopics = memoryEnvelopeHandedOver(envelope).map((entry) => entry.memory_id).slice(0, 20);
...
export function recordCoreCliRecall({
  ...
  const completed = createMemoryObservabilityEvent({
    ...
    payload: {
      ...
      // This surface reports whether the CLI abstained, not why: it is assembled from a ledger
      // replay that carries no envelope. Naming a reason here would invent one.
      abstain_reason: null,
