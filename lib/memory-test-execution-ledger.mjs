 ...
 */
export const KNOWN_TEST_RUNNERS = ['go-test', 'gradle-junit', 'node-self-test', 'swift-test', 'vitest', 'xctest'];
...
/**
 * The fields a verdict is actually derived from. `resolveTestExecution` grades a record by
 * comparing its `fingerprint` against the file on disk and reading its `result`; `run_id` and
 * `duration_ms` reach no verdict and no gate. Re-running an unchanged suite therefore produces a
 * record that is graded-identical to the one already stored and differs only in those two fields.
 */
export function sameGradedTestOutcome(previous, next) {
  if (!previous) return false;
  return GRADED_TEST_OUTCOME_FIELDS.every(field => (previous[field] ?? null) === (next[field] ?? null));
}

const GRADED_TEST_OUTCOME_FIELDS = [
  'runner', 'test_id', 'path', 'symbol', 'fingerprint', 'fingerprint_symbol',
  'result', 'aggregate', 'observed_tests', 'skipped_tests',
];
...
/**
 * Whether the roll-up already in the ledger keeps standing against the one this batch derived.
 *
 * A roll-up is a claim about the tests that ran, and a filtered rerun (`--tests *Chat*`) that
 * touches one case of a forty-case file derives a roll-up of one. Letting it replace the full-suite
 * record on identical code would let a memory citing the suite read `passed` on the strength of a
 * single case -- measured 2026-09-05: a 43-case Android roll-up shrank to `observed_tests: 1`, still
 * `passed`, after a `*Chat*` selection. So on the same fingerprint a passing roll-up that observed
 * fewer cases does not land. A failure always lands (one failing case fails the file), a changed
 * file always lands (the old record no longer describes the code), and a wider run always lands.
 */
function widerRollUpStands(existing, entry) {
  return Boolean(existing)
    && existing.aggregate && entry.aggregate
    && existing.fingerprint === entry.fingerprint
    && entry.result === 'passed'
    && existing.observed_tests > entry.observed_tests;
}
...
export function ingestTestRun({
  ...
  // A runner is known when the ledger lists it or the caller brought an adapter for it, so a
  // project can connect a toolchain of its own without editing this list.
  if (!KNOWN_TEST_RUNNERS.includes(run.runner) && !adapters[run.runner]) {
    throw new Error(`unknown test runner: ${run.runner} (known: ${[...new Set([...KNOWN_TEST_RUNNERS, ...Object.keys(adapters)])].join(', ')})`);
  }
  ...
  for (const record of [...records, ...deriveTestFileAggregates(records)]) {
    ...
    const key = testExecutionResultKey(entry);
    if (widerRollUpStands(results[key], entry)) {
      narrower.push(entry.test_id);
      continue;
    }
    // `changed` is the honest answer to "did this batch teach the ledger anything". A record whose
    // graded fields all match the one already stored is the same evidence observed a second time:
    // keeping the older entry loses no verdict, while replacing it rewrites `run_id`/`duration_ms`
    // across the file. That rewrite is what made a git-tracked ledger dirty after every test run.
    //
    // The older entry is also what gets kept when this batch does change something else. Replacing
    // every re-observed record whenever one record moved turned a one-suite change into a
    // whole-file diff of timings (measured 2026-09-15: 69 lines of `run_id`/`duration_ms` around a
    // handful of real fingerprint moves).
    stored.push(entry);
    if (sameGradedTestOutcome(results[key], entry)) continue;
    changed.push(entry);
    results[key] = entry;
  }
  const runEntry = {
  ...
  return { ledger: next, run: runEntry, stored, changed, unresolved, uncited, narrower };
}
...
export function findTestExecutionRecord(ledger, evidence) {
  ...
  if (evidence.path) {
    ...
    // A symbol that names the file's own type (`FooTests` for `FooTests.swift`) is a suite-level
    // anchor: the claim is "this suite ran", no runner reports a case by that name, and the file
    // roll-up is precisely the record that answers it.
    if (evidence.symbol && evidence.symbol === path.basename(evidence.path, path.extname(evidence.path))) {
      const aggregate = ledger.results[evidence.path];
