...
export const DEFAULT_DISMISSAL_RECEIPT_FILE = '.local-test/memory-miss-dismissal-receipts.jsonl';
...
export function pendingTriggerBackfills({
  ...
  dismissalFile = DEFAULT_DISMISSAL_RECEIPT_FILE,
} = {}) {
  ...
  const resolved = new Set(readMissResolutionReceipts(path.resolve(root, resolutionFile))
    .entries.map(entry => entry.feedback_line));
  const dismissed = new Map(readMissDismissalReceipts(path.resolve(root, dismissalFile))
    .entries.map(entry => [entry.feedback_line, entry]));
  ...
  for (const feedback of inbox.entries) {
    ...
    // A person has already graded this one and said the trigger lane cannot reach it. Re-grading it
    // every run produced 57 consecutive blocked runs against a single proposal here, which reads as
    // a broken runner rather than as the refusal it actually was.
    if (dismissed.has(feedback.line)) {
      excluded.push({
        line: feedback.line,
        reason: `dismissed-by-a-person:${dismissed.get(feedback.line).reason}`,
      });
      continue;
    }
