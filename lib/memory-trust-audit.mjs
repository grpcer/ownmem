...
export function collectMemoryTrustAudit({
  ...
  let quarantined = 0;
  let drifted = 0;
  // Split out so the handful of drifts that need a decision are not printed identically to the
  // structural ones; see the comment where they are classified.
  let preciseDrifted = 0;
  let coarseDrifted = 0;
  let coarseAnchors = 0;
  let misanchoredDrifted = 0;
  let evidenceValid = 0;
  ...
  for (const topic of topics) {
    ...
    if (!trust.valid) {
      ...
      if (driftReasons.length > 0) {
        drifted += 1;
        const anchors = trust.evidence.advisory.filter(item => !TEST_EXECUTION_REASONS.includes(item.reason));
        // Two kinds of drift arrive under one reason, and printing them the same way is what buried
        // the one that needs a person. A symbol fingerprint moved because that symbol's own
        // implementation changed -- somebody has to decide whether the lesson survived it. A coarse
        // anchor (a whole file, or a canonical document) moves on any edit anywhere in the file, so
        // it says nothing about the lesson, and on this corpus it is nine drifts in ten. Re-signing
        // clears it until the next unrelated edit, which is a treadmill, not a fix; the fix is to
        // name the symbol the memory is actually about.
        //
        // Measured 2026-09-18 over the whole corpus: 66 drifted topics, 71 coarse anchors
        // (39 document, 22 path, 10 test) against 8 symbol fingerprints in 7 topics.
        const precise = anchors.filter(item => item.reason === 'fingerprint-drift');
        // A third case, and re-signing is the wrong advice for it: the slice matched a call site or
        // a comment instead of a definition, so the receipt is fingerprinting the wrong block. That
        // is a broken anchor, not a moved one, and `--refresh-evidence` would happily re-sign the
        // comment. The fix is to name a symbol that is actually defined in that file.
        const misanchored = anchors.filter(item => item.reason === 'symbol-not-a-definition');
        if (precise.length > 0) {
          preciseDrifted += 1;
          output.push(issue('warning', 'trust-evidence-drift', `${topic.record.name} evidence drifted at a named symbol, downgraded to advisory: ${precise.map(item => item.locator).join(', ')}; read the symbol and decide whether the lesson survived, then run: ${issueCommand} ${topic.record.name} --refresh-evidence`, precise.map(item => `${item.kind}:${item.locator}:${item.reason}`)));
        }
        if (misanchored.length > 0) {
          misanchoredDrifted += 1;
          output.push(issue('warning', 'trust-evidence-misanchored', `${topic.record.name} anchors a symbol that is not defined where it points: ${misanchored.map(item => item.locator).join(', ')}; the fingerprint is tracking a call site or a comment, so re-signing would only re-sign the wrong block — point it at a symbol defined in that file`, misanchored.map(item => `${item.kind}:${item.locator}:${item.reason}`)));
        }
        if (precise.length === 0 && misanchored.length === 0) {
          coarseDrifted += 1;
          coarseAnchors += anchors.length;
        }
      }
  ...
  // One line for the structural drift, because a per-topic list of it is noise that hides the rest
  // of this report. The remedy named here is the one that actually ends it: an anchor that names the
  // symbol the memory is about stops moving when something else in the same file does.
  if (coarseDrifted > 0) {
    output.push(issue('warning', 'trust-evidence-drift-coarse', `${coarseDrifted} topic(s) drifted only at whole-file or document anchors (${coarseAnchors} anchor(s)), downgraded to advisory; these move on any edit to the file and re-signing them only postpones the next one — give the memory a symbol-level anchor instead, or leave it advisory`));
  }
  ...
  return {
    ...
    summary: {
      ...
      quarantined,
      drifted,
      drifted_precise: preciseDrifted,
      drifted_misanchored: misanchoredDrifted,
      drifted_coarse: coarseDrifted,
