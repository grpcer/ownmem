...
// A path that looks like it holds tests. Used to gate the weak match below; deliberately about the
// file's own name and location rather than its contents, because contents are what the weak match
// already looks at.
function looksLikeTestFile(relative) {
  const stem = path.basename(relative, path.extname(relative));
  return /tests?$/iu.test(stem)
    || /^test/iu.test(stem)
    || /_test$/iu.test(stem)
    || /(^|\/)(tests?|__tests__|spec)(\/)/iu.test(relative);
}
...
// a file that merely contains the case name as an identifier is accepted too, for test suites whose
// file is not named after the type.
//
// That second match is gated on the file looking like a test file, and the gate is not optional. In
// this corpus `code_evidence` routinely reads "this production file, covered by these tests", so
// the declared path is usually the production file -- and production files mention their own test
// suites in comments. Measured 2026-09-18, an ungated weak match bound
// `BlockCommitTextWriter.swift#CanonicalSpanRenderTests` to a doc comment on line 13, so the
// receipt fingerprinted a comment in the production file instead of the test that vouches for it,
// and nothing said so. Failing the weak match here falls through to the repository-wide lookup,
// which finds the real test file.
function declaredTestFile(root, declaredPath, className, symbol) {
  ...
  if (!looksLikeTestFile(declared.relative)) return null;
