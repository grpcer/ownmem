...
export function memoryTrustVerdicts({ document, queries, conflicts = [], maxStalenessDays, now = new Date(), trust = null }) {
  ...
  const grantingAuthority = ['normative', 'user-confirmed'].includes(authority);
  const normativeHighRisk = highRiskAction && grantingAuthority;
  ...
  const advisoryHighRisk = risky && !negated && highRiskAction && !grantingAuthority;
