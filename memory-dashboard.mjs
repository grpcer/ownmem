...
} from './lib/memory-dashboard-server.mjs';

const SCRIPT_DIR = path.dirname(fileURLToPath(import.meta.url));
const DEFAULT_ROOT = path.resolve(SCRIPT_DIR, '..');
const DEFAULT_MEMORY_DIRECTORY = '.claude/memory';
...
export async function runCli(rawArgs = process.argv.slice(2), io = { stdout: process.stdout, stderr: process.stderr }) {
  ...
  const dashboard = createMemoryDashboardServer({ root: options.root, memoryDir: options.memoryDir, token });
