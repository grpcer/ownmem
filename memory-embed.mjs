...
} from './lib/memory-compiler.mjs';
import {
  evaluateMemoryEmbeddingAb,
  MEMORY_EMBEDDING_SUGGESTED_WEIGHT,
} from './lib/memory-embedding-evaluation.mjs';
...
Global options:
  ...
  --index-dir PATH   Local compiled memory index (default .local-test/memory-index)
...
function checkpointEmbeddingBatch(state, { root, directory, config, now }, { offset, vectors, dimensions }) {
...
async function runAbCommand(options, io, dependencies, compile) {
  const result = await evaluateMemoryEmbeddingAb({
    ...
    sleepImpl: dependencies.sleepImpl,
    compile,
    casesFile: dependencies.casesFile,
