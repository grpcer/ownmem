...
import { collectMemoryHealth } from './memory-health.mjs';
import {
  DEFAULT_MEMORY_OBSERVABILITY_DIRECTORY,
  MEMORY_RECALL_COMPONENTS,
  readMemoryObservabilityEvents,
} from './lib/memory-observability.mjs';
import { readMemoryEmbeddingConfig } from './lib/memory-embedding-provider.mjs';
import { MEMORY_RECALL_RUNTIME_VERSION } from './lib/memory-runtime.mjs';

const SCRIPT_DIR = path.dirname(fileURLToPath(import.meta.url));
const DEFAULT_ROOT = path.resolve(SCRIPT_DIR, '..');
const SMALL_SAMPLE_THRESHOLD = 20;
const REPORT_SCHEMA = JSON.parse(readFileSync(path.join(SCRIPT_DIR, 'memory-report.schema.json'), 'utf8'));
...
function currentEmbeddingState(root) {
  try {
    ...
    const mode = !config.enabled ? 'off' : config.rrf_weight > 0 ? 'weighted' : 'observe';
    return {
      configured: true,
      mode,
...
function staticGateHealth(root, memoryDir, health) {
  ...
  const quotaSchemaHealthy = quota.valid && ['ownmem.quota/v2', 'ownmem.quota/v3', 'oriveo.memory.quota/v2'].includes(quota.value.schema);
  ...
  const qualityHealthy = quality.valid
    ...
    && (quality.value.schema.startsWith('ownmem.quality/') || quality.value.schema.startsWith('oriveo.memory.quality/'));
  return {
    schema: health ? {
...
function recommendations(report) {
  ...
  if (report.data_quality.invalid_events > 0) {
    add('fix-invalid-events', 100, 'high', 'low', `${report.data_quality.invalid_events} invalid event line(s)`, 'Fix or quarantine the invalid events so the report stops underreporting real traffic.');
  }
  ...
  // improvement already fixed a miss -- deciding that requires actually re-running the recall, which
  // only memory-feedback-review does. So the wording points at that tool instead of asserting a
  // backlog: on 2026-08-15 the single logged miss showed up here as "1/1 wrong or miss" while the
  // review queue had already resolved it and reported 0 actionable.
  if (report.quality.feedback.total > 0 && report.quality.feedback.wrong_miss_rate > 0.1) {
    add('review-feedback', 70, 'high', 'medium', `${report.quality.feedback.wrong + report.quality.feedback.miss}/${report.quality.feedback.total} logged wrong or miss (resolved ones not deducted here)`, 'Run memory-feedback-review: it re-runs each recall and drops the ones a later improvement already fixed, leaving only what still needs triggers, samples or ranking rules.');
  }
...
export function collectMemoryReport({
  ...
  const feedbackCounts = { correct: 0, wrong: 0, miss: 0 };
  feedback.forEach(event => increment(feedbackCounts, event.payload.verdict));
  ...
  if (eventRead.errors.length > 0) gap('invalid-events', `${eventRead.errors.length} invalid event(s); the report may underreport traffic.`);
  if (healthError) gap('health-read-failed', `Static health read failed: ${healthError}`);

  const report = {
    schema: 'ownmem-report/v5',
    ...
    metric_roles: {
      ...
      actual_application_evidence: 'unavailable',
    },
    ...
    quality: {
      feedback: {
        ...
        ...feedbackCounts,
        wrong_miss_rate: ratio(feedbackCounts.wrong + feedbackCounts.miss, feedback.length),
        ...
        // Two months of real use produced zero `wrong` (only one `miss`, a different verdict).
        //
    ...
    data_quality: { files: eventRead.files.length, invalid_events: eventRead.errors.length, event_errors: eventRead.errors.slice(0, 10), gaps: dataGaps.map(item => item.text), gap_codes: dataGaps.map(item => item.code) },
    recommendations: [],
  };
...
export function formatMemoryReport(report) {
  ...
  const lines = [
    ...
    `North star (recall.consumed, stack ${report.delivery.runtime_version}): produced ${report.delivery.produced_traces} -> delivered ${report.delivery.delivered_traces} (${percentage(report.delivery.delivery_rate)}) -> confirmed full-text opens ${report.delivery.fulltext_open_traces} (${percentage(report.delivery.fulltext_open_rate)}); actual application rate unavailable (whether the envelope summary itself solved the problem is still not observable). A low open rate is expected: the 400-token envelope is built so the caller rarely needs the source, so this is a coverage floor, not an adoption rate`,
    ...
    `Quality: offline Recall@1/Recall@5/MRR are process gates only; feedback correct/wrong/miss=${report.quality.feedback.correct}/${report.quality.feedback.wrong}/${report.quality.feedback.miss}; accuracy is not reported — ${report.quality.feedback.accuracy_unmeasurable_reason}`,
