...
import { loadMemoryTopics, parseMemoryContent } from './lib/memory-schema.mjs';
...
function check(options) {
  ...
  for (const topic of topics) {
    ...
    if (!receipt) {
      errors.push(`${name}: review receipt is missing; run memory-review.mjs bootstrap or confirm`);
      continue;
    }
...
if (isMemoryCliEntry(import.meta.url)) {
  try {
    ...
    process.stderr.write(`memory-review: ${error.message}\n`);
    process.exitCode = 1;
  }
}
